...
public class BotoConfig  extends Abstract implements CredentialHandler {
    ...
    private static final String BOTO_CONFIG_PEGASUS_PROFILE_KEY = BotoConfig.BOTO_CONFIG_FILE_VARIABLE.toLowerCase() ;//has to be lowercased
     ...
     *
     * - If a BOTO_CONFIG is specified as a Pegasus Profile in the site catalog
    ...
    public  String getPath( String site ){
        ...
        String path = ( siteEntry == null )? null :
                       (String)siteEntry.getProfiles().get( Profiles.NAMESPACES.pegasus).get( BotoConfig.BOTO_CONFIG_FILE_VARIABLE.toLowerCase() );
    ...
    public String getLocalPath(){
        ...
        String cred = ( siteEntry == null )? null :
                        (String)siteEntry.getProfiles().get( Profiles.NAMESPACES.pegasus).get( BotoConfig.BOTO_CONFIG_PEGASUS_PROFILE_KEY );
        if( cred == null && siteEntry != null ){
            //try to check for an env profile in the site entry 
            cred = (String)siteEntry.getProfiles().get( Profiles.NAMESPACES.env).get( BotoConfig.BOTO_CONFIG_FILE_VARIABLE );
        }
