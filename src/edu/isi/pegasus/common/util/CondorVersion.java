...
public class CondorVersion {
    ...
    /** Predefined Constant for condor version 8.9.4 */
    public static final long v_8_9_4 = CondorVersion.numericValue("8.9.4");
    ...
    /**
     * Store the regular expressions necessary to parse the output of condor_version. The rule for
     * the format is
     *
     * <p>$CondorVersion: 7.4.1 Dec 17 2009 <ANY_ARBITRARY_STRING> $ where <ANY_ARBITRARY_STRING>
     * may or may not be there, and can include spaces but is really completely arbitrary.
     *
     ...
     *
     * @param version in form of major.minor.patch. You can opt to omit the minor and patch versions
     *     if you want
     * @return float value of the version, else -1 in case of null version or incorrect formatted
     *     string
     */
        ...
        try {
            ...
            for (int z = 0; z < y; z++) {
                ...
                if (len > CondorVersion.MAX_VERSION_PRECISION) {
                    throw new IllegalArgumentException(
                            "Only two digit precision is allowed in version numbers " + version);
                }
                ...
                for (int d = 0; d < len; d++) {
                    ...
                    if (!Character.isDigit(ch)) {
                        throw new IllegalArgumentException(
                                "Non digit specified in version " + version);
                    }
                    arr[i++] = ch;
                }
            }
    ...
    public long numericValue() {
        ...
        try {
            ...
            mLogger.log(
                    "Exception while parsing condor_version ", e, LogManager.ERROR_MESSAGE_LEVEL);
        }
        return result;
    }
    ...
    public String version() {
        ...
        try {
            ...
            if (status != 0) {
                mLogger.log(
                        "Command " + CONDOR_VERSION_COMMAND + " exited with status " + status,
                        LogManager.WARNING_MESSAGE_LEVEL);
            }

        } catch (IOException ioe) {
            mLogger.log(
                    "IOException while determining condor_version ",
                    ioe,
    ...
    /**
     * An inner class, that implements the StreamGobblerCallback to determine the version of Condor
     * being used.
     */
