...
/**
 * This class solely defines the version numbers of PEGASUS. The template file will be substituted
 * by ant during the built process to compile in the built timestamp.
 *
 * <p>When using the class, the methods for the version number digits will always return an integer.
 * In order to obtain the full version number, including any "release candidate" suffices, please
 * prefer the <code>toString</code> method over combining the separate version integers yourself.
 *
 * @author Karan Vahi
...
public class Version {
    ...
    public Version() {
        try {
            props.load(
                    Version.class.getClassLoader().getResourceAsStream("pegasus.build.properties"));
    ...
    /**
     * Determines the built and architecture of the installation. These are usually separated by a
     * linear white-space.
     *
     * @return the formatted time stamp of the built, if available, and an identifier for the
     *     architecture. An string indicating that the build is unknown is returned on failure.
     */
    ...
    /**
     * Determines, if the compiled version and the installed version match. The match is done by
     * comparing the timestamps and architectures.
     *
