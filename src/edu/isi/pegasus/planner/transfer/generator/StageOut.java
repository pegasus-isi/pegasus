...
/**
 * This class determines where the outputs generated by a job  need to be placed
 * It computes FileTransfer pairs that contain both the source URL on the staging 
 * site and the destination URL on the output site. Internally the StageOut Placer
 * relies on OutputMapper to find the destination URL on the output site. 
 * It also groups File Transfers as transfers that 
 *   -  need to be managed locally (third party mode) IR
 *   -  remotely (case where the staging site only has file accessible URLs)
 *
 * @author Karan Vahi
 */
public class StageOut extends Abstract{
     ...
     */
    public Collection<FileTransfer> getDeletedFileTX(ReplicaCatalogBridge rcBridge, ReplicaSelector replicaSelector, Job job, String destSite) {
    ...
    /**
     * This determines where output files for a job should be placed on an output site
     * If the transfer transient flag is set, it means the file does not have to be transferred to
     * the output site.
     *
     ...
     */
    public Collection<FileTransfer>[] getFileTX(
            Job job, String destSiteHandle ) {
    ...
    private NameValue<Boolean, FileTransfer> constructFileTX(
        ...
        String sharedScratchGetURL =
                this.getURLOnSharedScratch(stagingSite, job, FileServerType.OPERATION.get, addOn, lfn);
        String sharedScratchPutURL =
                this.getURLOnSharedScratch(stagingSite, job, FileServerType.OPERATION.put, addOn, lfn);
