...
/**
 * An extension of the default refiner, that allows the user to specify the
 * number of transfer nodes per execution site for stagein and stageout. The
 * files are distributed in a round robin manner across the stagein and stageout
 * jobs. Currently it is per workflow for the stage-in while for stageout it is
 * per level of the workflow.
 *
 * @author Karan Vahi
 * @version $Revision$
 */
public class BalancedCluster extends Basic {
    ...
    /**
     * The map indexed by staged executable logical name. Each entry is the name
     * of the corresponding setup job, that changes the XBit on the staged file.
     */
    protected Map mSetupMap;

    /**
     * A map indexed by site name, that contains the pointer to the local stage
     * out PoolTransfer objects for that site. This is per level of the
     * workflow.
     */
    private Map<String, PoolTransfer> mStageOutLocalMapPerLevel;

    /**
     * A map indexed by site name, that contains the pointer to the remote stage
     * out PoolTransfer objects for that site. This is per level of the
     * workflow.
     */
     ...
     *
     * The factor is computed by looking up the pegasus profiles in the
     * properties.
     * <pre>
    ...
    /**
     * Adds the stage in transfer nodes which transfer the input files for a
     * job, from the location returned from the replica catalog to the job's
     * execution pool.
     *
     * @param job   <code>Job</code> object corresponding to the node to which the
     * files are to be transferred to.
     * @param files Collection of <code>FileTransfer</code> objects containing
     * the information about source and destURL's.
     * @param symlinkFiles Collection of <code>FileTransfer</code> objects
     * containing source and destination file url's for symbolic linking on
     * compute site.
     */
     ...
     * @param job   <code>Job</code> object corresponding to the node to which the
     * files are to be transferred to.
     * @param localTransfer boolean indicating whether transfer has to happen on
     * local site.
     * @param files Collection of <code>FileTransfer</code> objects containing
     * the information about source and destURL's.
     * @param type the type of transfer job being created
     * @param stageInMap Map indexed by site name that gives all the transfers
     * for that site.
     * @param bundleValue used to determine the bundling factor to employ for a
     * job.
     ...
     * @param deletedLeaf to specify whether the node is being added for a
     * deleted node by the reduction engine or not. default: false
     */
     ...
     * @param localTransfer indicates whether transfer job needs to run on local
     * site or not.
     * @return
     */
     ...
     * @param localTransfer whethe the associated transfer job runs on local
     * site or remote.
     ...
     * @param localTransfer whether the transfer jobs need to run on local site
     * or not
     *
    ...
    /**
     * Builds a map that maps for each level the number of default transfer jobs
     * to be created
     *
     * @param divisor
     * @return
    ...
    /**
     * Builds up a message for logging a scaling message indicating user to
     * remove a property
     *
     * @param key
     */
    ...
    protected class TransferContainer {
        ...
        /**
         * Returns the collection of registration files associated with this
         * transfer container.
         *
         * @return a collection of <code>FileTransfer</code> objects.
         */
    ...
    protected class PoolTransfer {
        ...
        /**
         * The index of the job to which the next transfer for the pool would be
         * scheduled.
         */
        private int mNext;
        ...
        /**
         * The list of <code>TransferContainer</code> that correspond to each
         * transfer job.
         */
        private List mTXContainers;

        /**
         * boolean indicating whether the transfer job needs to run on local
         * site
         */
         ...
         * @param localTransfer whether the transfers need to be run on local
         * site
         * @param number the number of transfer jobs that are going to be
         ...
         * TransferContainer, and the pointer is then updated to the next
         * container.
         *
         ...
         *
         * @return the Transfer Container to which the job file transfers were
         * added.
         */
        ...
        /**
         * Adds a single <code>FileTransfer</code> objects to the appropriate
         * TransferContainer. The collection is added to a single
         * TransferContainer, and the pointer is then updated to the next
         * container.
         *
         * @param file
    ...
    protected class ClusterValue {
         ...
         *
         * @param key the Pegasus Profile key to be used for lookup of bundle
         * values.
         * @param defaultKey the default Profile Key to be used if key is not
         * found.
         * @param defaultValue the default value to be associated if no key is
         * found.
         */
        ...
        /**
         * Determines the bundle factor for a particular site on the basis of
         * the stage in bundle value associcated with the underlying transfer
         * transformation in the transformation catalog. If the key is not
         * found, then the default value is returned. In case of the default
         * value being null the global default is returned.
         *
         * The value is tored internally to ensure that a subsequent call to
         * get(String site) returns the value determined.
         *
         * @param implementation the transfer implementation being used
         * @param job the compute job for which the bundle factor needs to be
         * determined.
         *
         ...
         * value being null the global default is returned.
         *
         * The value is stored internally to ensure that a subsequent call to
         * get(String site) returns the value determined.
         *
         * @param implementation the transfer implementation being used
         * @param job the compute job for which the bundle factor needs to be
