...
/**
 * A factory class to load the appropriate type of SLS Implementation to do the Second Level
 * Staging.
 *
 * @author Karan Vahi
 * @version $Revision$
...
public class SLSFactory {
    ...
    public static final String DEFAULT_PACKAGE_NAME = "edu.isi.pegasus.planner.transfer.sls";
    ...
    public static String[] SLS_IMPLEMENTING_CLASSES = {"Transfer", "Condor"};
    ...
    public SLS loadInstance(Job job) throws SLSFactoryException {
        ...
        // sanity checks first
        if (!mInitialized) {
            throw new SLSFactoryException("SLSFactory needs to be initialized first before using");
        }
     ...
     * @throws SLSFactoryException
     */
    private SLS loadInstance(PegasusBag bag, String className) throws SLSFactoryException {
    ...
    private String getSLSShortName(Job job) {
        ...
        return (conf != null
                        && (conf.equalsIgnoreCase(PegasusConfiguration.CONDOR_CONFIGURATION_VALUE)))
