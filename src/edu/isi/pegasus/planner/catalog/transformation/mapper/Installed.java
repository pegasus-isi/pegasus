...
public class Installed extends Mapper {
    ...
    /**
     * A map that maps a transformation name to the set of sites where it is not installed. It is
     * used to prevent duplicate queries in case of a transformation not installed on a particular
     * site.
     */
        ...
        if (sysinfomap != null) {
            for (Iterator i = siteids.iterator(); i.hasNext(); ) {
                ...
                for (Iterator j = tcentries.iterator(); j.hasNext(); ) {
                    ...
                    if (txsysinfo.equals(sitesysinfo) && txsiteid.equalsIgnoreCase(site)) {
