...
/**
 * This interface describes a minimum set of essential tasks required from a
 * replica catalog. The method verbs have a steering meaning associated with
 * them:
 *
 * <dl>
 * <dt>lookup</dt>
 * <dd>retrieves physical filenames or replica entries</dd>
 * <dt>list</dt>
 * <dd>retrieves only lists of logical filenames</dd>
 * <dt>delete</dt>
 * <dd>removes an entry specified by LFN and PFN</dd>
 * <dt>remove</dt>
...
public interface ReplicaCatalog extends Catalog {
     ...
     * Retrieves multiple entries for a given logical filename, up to the
     * complete catalog. Retrieving full catalogs should be harmful, but may be
     * helpful in online display or portal.<p>
     *
     * @param lfns is a set of logical filename strings to look up.
     ...
     * name, e.g. the resource handle 'site', to a string that has some meaning
     * to the implementing system. This can be a SQL wildcard for queries, or a
     * regular expression for Java-based memory collections. Unknown keys are
     * ignored. Using an empty map requests the complete catalog.
     * @return a map indexed by the LFN. Each value is a collection of replica
     * catalog entries.
     * @see ReplicaCatalogEntry
     */
     ...
     *
     * @param constraint is a constraint for the logical filename only. It is a
     * string that has some meaning to the implementing system. This can be a
     * SQL wildcard for queries, or a regular expression for Java-based memory
     * collections.
