 ...
 * information from kickstart generated invocation record.
 *
...
public class InvocationParser extends DefaultHandler {
    ...
    public InvocationParser(String schemaLocation) {
        ...
        try {
            ...
            if (schemaLocation != null) {
                setSchemaLocations(InvocationRecord.SCHEMA_NAMESPACE + ' ' + schemaLocation);
    ...
    /**
     * This function parses a XML source from the new Reader source, and creates java class
     * instances that correspond to different elements in the XML source.
     *
    ...
    public void startElement(
            ...
            Attributes atts)
            throws SAXException {
        m_log.log("parser", 3, "<" + map(namespaceURI) + localName + "> at " + where());
        ...
        if (object != null) m_stack.push(new IVSElement(qName, object));
        ...
        if (!m_stack.empty()) {
            ...
            if (!setElementRelation(peek.m_name.charAt(0), peek.m_obj, tos.m_obj)) {
                m_log.log(
                        "parser",
                        0,
                        "Element " + tos.m_name + " does not fit into element " + peek.m_name);
    ...
    public void characters(char[] ch, int start, int length) throws SAXException {
        ...
        if (message.length() > 0) {
            if (message.trim().length() == 0)
                m_log.log("parser", 3, "Characters: \' \' x " + message.length());
    ...
    private void log(String subject, String name, String value) {
        ...
        m_log.log("filler", 3, subject + "." + name + "=\"" + value + "\"");
    }
     ...
     */
    protected Invocation createObject(
            Invocation parent, String e, java.util.List names, java.util.List values)
        ...
        try {
            ...
            m_log.log(
                    "filler",
                    0,
                    "Error: Unable to parse a number: " + nfe.getMessage() + " at " + where());
            return null;
        } catch (UnknownHostException uh) {
            m_log.log(
                    "filler",
                    0,
                    "Error: Unable to parse a hostname: " + uh.getMessage() + " at " + where());
            return null;
        } catch (ParseException pe) {
            m_log.log(
                    "filler",
                    0,
                    "Error: Unable to parse a date: " + pe.getMessage() + " at " + where());
            return null;
        }
    }
    ...
    protected boolean setElementRelation(char initial, Invocation parent, Invocation child) {
        switch (initial) {
            ...
            default:
                // FIXME: shouldn't this be an exception?
                m_log.log("filler", 0, "Error: unable to join child to parent");
                return false;
        }
    }
}
