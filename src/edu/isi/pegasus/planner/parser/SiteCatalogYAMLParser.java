...
public class SiteCatalogYAMLParser {
    ...
    /**
     * This code is to parse any format to YAML.. Here we iterate the store results and convert it
     * to the map format and finally write to the specified file.. *
     */
        ...
        try {
            ...
            while (siteIterator.hasNext()) {
                    ...
                    if (!fileserversInfo.isEmpty()) {
                        ...
                        for (FileServer fileserverInfo : fileserversInfo) {
                            ...
                            if (fileserverInfo.getProfiles() != null) {
                                List<Map<String, Map<String, Object>>> profileData =
                                        buildProfiles(fileserverInfo.getProfiles().getProfiles());
                                if (profileData != null && profileData.size() > 0) {
                                    fileServerMap.put("profile", profileData);
                                }
                                Map<String, Object> metaData =
                                        buildMeta(fileserverInfo.getProfiles().getProfiles());
                ...
                while (gridGateWayIter.hasNext()) {
                    ...
                    edu.isi.pegasus.planner.catalog.site.classes.GridGateway.TYPE type =
                            gatewayentry.getType();
                    if (type != null) {
                ...
                if (entry.getProfiles() != null) {
                    List<Map<String, Map<String, Object>>> profileData =
                            buildProfiles(entry.getProfiles().getProfiles());
                    if (profileData != null && profileData.size() > 0) {
