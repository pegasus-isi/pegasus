...
/**
 * This class is an abstract class for the Transformation Catalog Selector. Its purpose is to
 * provide a generic api to select one valid transformation among the many transformations.
 *
...
public abstract class TransformationSelector {
    ...
    public static final String PACKAGE_NAME = "edu.isi.pegasus.planner.selector.transformation";
     ...
     * The Random and RoundRobin implementation ensure that only one entry is returned and should be
     * run last when chaining multiple selectors
     *
     * @param tcentries List
     * @param preferredSite the preferred site for selecting the TC entries
     * @return List
     */
    public abstract List getTCEntry(
            List<TransformationCatalogEntry> tcentries, String preferredSite);

    /**
     * Loads the implementing class corresponding to the mode specified by the user at runtime in
     * the properties file.
     *
     * @param className String The name of the class that implements the mode. It is the name of the
     *     class, not the complete name with package. That is added by itself.
     * @return TransformationSelector
     * @throws FactoryException that nests any error that might occur during the instantiation of
     *     the implementation.
     */
    public static TransformationSelector loadTXSelector(String className) throws FactoryException {
