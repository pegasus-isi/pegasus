...
public class GridStartFactory {
    ...
    /**
     * Initializes the <code>POSTScript</code> implementation table, associating short names for the
     * POSTScript with the name of the classes itself.
     */
    static {
    ...
    private static void associate(POSTScript ps) {
        POSTSCRIPT_IMPLEMENTING_CLASS_TABLE.put(ps.shortDescribe(), ps.getClass().getName());
    }
     ...
     *
     * @param shortName the shortName for the POSTScript implementation
     * @return the className the fully qualified className of the implementing class, else null.
     */
    ...
    /**
     * The postscript mode. Whether to add postscripts for the jobs or not. At present just two
     * modes supported all add postscripts for jobs where kickstart is present. none do not add
     * postscripts to anyjob
     */
    //    private String mPostScriptScope;

    /**
     * A table that maps short names of <code>POSTScript</code> implementations with the
     * implementations themselves.
     */
    private Map mPOSTScriptImplementationTable;

    /**
     * A table that maps short names of <code>GridStart</code> implementations with the
     * implementations themselves.
     */
    ...
    public void initialize(PegasusBag bag, ADag dag, String postScriptLog) {
        ...
        for (int i = 0; i < GRIDSTART_IMPLEMENTING_CLASSES.length; i++) {
            ...
            registerGridStart(
                    GRIDSTART_SHORT_NAMES[i],
                    this.loadGridStart(bag, dag, GRIDSTART_IMPLEMENTING_CLASSES[i]));
        }
     ...
     */
    public GridStart loadGridStart(Job job, String gridStartPath) throws GridStartFactoryException {
    ...
    /**
     * Returns the short name for the gridstart implementation that needs to be loaded for the job.
     *
     * @param job
     * @return
     */
     ...
     */
    private GridStart loadGridStart(PegasusBag bag, ADag dag, String className)
            throws GridStartFactoryException {
        ...
        try {
            ...
            throw new GridStartFactoryException("Instantiating GridStart ", className, e);
        }
     ...
     * @see #DEFAULT_PACKAGE_NAME
     */
    private POSTScript loadPOSTScript(
    ...
    /**
     * Returns the cached implementation of <code>POSTScript</code> from the implementing class
     * table.
     *
