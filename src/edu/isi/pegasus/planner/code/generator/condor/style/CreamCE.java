 ...
 *
 * <p>The CREAM CE support in Condor is documented at the following link
 *
 * <pre>
 * http://research.cs.wisc.edu/htcondor/manual/v7.9/5_3Grid_Universe.html#SECTION00637000000000000000
 * </pre>
 *
 * The protocol requires an X.509 proxy for the job, so the submit description file command
 * x509userproxy will be used.
 *
 * @author Karan Vahi
 * @version $Revision$
 */
public class CreamCE extends Abstract {
     ...
     * three fields.
     *
     * <p>A CREAM grid resource specification is of the form:
     *
     * <p>grid_resource = cream <web-services-address> <batch-system> <queue-name>
     *
     * <p>The <batch-system> is the name of the batch system that sits behind the CREAM server, into
     * which it submits the jobs. Normal values are pbs, lsf, and condor. It is picked up from the
     * scheduler attribute for the grid gateway entry in the site catalog entry for the site
     *
     * <p>The <queue-name> identifies which queue within the batch system should be used. Values for
     * this will vary by site, with no typical values and are picked up from the globus profile
    ...
    protected String constructGridResource(Job job) throws CondorStyleException {
        ...
        if (queue == null) {
            ...
            message.append("Globus Profile Queue ")
                    .append("queue")
