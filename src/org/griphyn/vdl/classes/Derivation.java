...
/**
 * <code>Derivation</code> is an implementation of an abstract VDL
 * <code>Definition</code>. A derivation describes the mutable part
 * concerning input, processing, and output (IPO) when calling an
 * application. The environment is part of the capture. 
 *
 * A derivation parametrizes the template provided by a
 * <code>Transformation</code> with actual values. Think of a derivation
 * as something akin to a C function call. The derivation provides the
...
{
  /**
   * Though most <code>Derivation</code>s may have a name of their own,
   * most of the times, though, derivations are anonymous. A derivation
   * must provide the name of the <code>Transformation</code> that
   * it calls, though.
   *
   * @see Definition
   * @see Transformation
  ...
  /**
   * The namespace in which a derivation resides can differ from the
   * namespace that the transformation lives in. This argument provides
   * the namespace of the <code>Transformation</code> to call. 
   * @see Definition
   * @see Transformation
   */
  ...
  /**
   * Any <code>Transformation</code> may exist in multiple versions. 
  ...
  /**
   * Actual arguments used when calling a {@link Transformation} are
  ...
  /**
   * Convenience ctor: Names a derivation and the used
   * <code>Transformation</code>
   * 
  ...
  /**
   * Convenience ctor: Names a derivation and supplies the used
   * <code>Transformation</code> as well as the permissable version
   * range.
   *
   * @param name is the name of the <code>Derivation</code>.
   * @param uses is the name of the <code>Transformation</code>.
  ...
  /**
   * Complete ctor: Constructs the full three part <code>Derivation</code>
   * identifier, and four part <code>Transformation</code> mapper.
   *
   ...
   * @see Transformation
   */
  public Derivation( String ns, String name, String version,
		     String us, String uses, String min, String max ) {
  ...
  /**
   * Convenience ctor: Names the derivation and supplies the used
   * <code>Transformation</code>, and the first actual argument.
   *
   * @param name is the name of the <code>Derivation</code>.
   * @param uses is the name of the <code>Transformation</code>.
  ...
  /**
   * Accessor: Obtains all actual arguments. The map is a read-only
  ...
  /**
   * Matches an external version string against the internal range. This
   * predicate function uses inclusive matches. Special interpretation
   * will be applied to <code>null</code> values, internally as well as
   * an external null value.<p>
   *
   * <pre>
   *   vers.   min    max     result
   *   -----   ----   -----   ------
   ...
   * </pre>
   *
   * @param version is an externally supplied version to be checked,
  ...
  public static boolean match( String version, String minInc, String maxInc )
  {
    ...
    if ( minInc == null && maxInc == null || version == null )
      return true;
  ...
  /**
   * Instance method for matching an external version against the inclusive
   * version range.
   * @param version is an externally supplied version to be checked,
   * if it is within the inclusive interval of min and max.
   * @return true, if the version is in range, false otherwise.
  ...
  {
    return Derivation.match( version, 
			     this.m_minIncludeVersion, 
			     this.m_maxIncludeVersion );
  }
  ...
  /**
   * Accessor: Sets the maximum inclusive permissable version of
  ...
  /**
   * Accessor: Sets the minimum inclusive permissable version of
   * a logical transformation to run with.
   *
  ...
  /**
   * Accessor: Replaces the bag of actual argument with a bag of 
   * new arguments.
   *
  ...
  /**
   * Accessor: Replaces the bag of actual argument with a map of 
   * new arguments.
   *
  ...
  /**
   * Accessor: Sets a new name for a logical <code>Transformation</code>
   * to call.
   * 
  ...
  /**
   * Accessor: Sets a new namespace identifier for a logical
   * <code>Transformation</code> to call.
   *
   * @param usesspace is the new namespace of the
   * <code>Transformation</code>.
   * @see #getUsesspace()
   * @see Transformation
   */
   ...
   * identifier for this derivation. Recent modification add the
   * complete identification in terms of derivation name, namespace,
   * and version as well as the called transformation name, namespace
   * and version range. 
  ...
  public String identify()
  {
    ...
    result.append("->");
    result.append( Separator.combine( this.m_usesspace, this.m_uses,
...
//     }
    ...
    throws IOException
  {
    ...
    String tag = ( namespace != null && namespace.length() > 0 ) ?
      namespace + ":derivation" : "derivation";
