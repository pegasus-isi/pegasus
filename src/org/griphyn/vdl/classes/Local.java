...
public class Local extends VDL implements Serializable
{
  ...
  /**
   * Each temporary variable has a specific type. There are currently
   * only the type of {@link Scalar} and {@link List}.
   */
  private int m_containerType = Value.SCALAR;

  /**
   * For linking the DAG we need to know if the argument is passed into
   * the transformation, produced by the transformation, or has some 
   * other behavior. However, temporary variables are usually transient,
   * and thus either of type none or inout. 
   */
  ...
  /**
   * ctor: Construct a temporary variable with a binding and default
   * container type.
   *
   * @param name is the binding.
   * @param ct is the container type, the type of the argument.
   * @throws IllegalArgumentException if the container type is outside
   * the legal range [{@link Value#SCALAR}, {@link Value#LIST}].
   */
  public Local( String name, int ct )
    throws IllegalArgumentException
  {
    super();
    this.m_name = name;
    if ( Value.isInRange(ct) ) 
      this.m_containerType = ct;
    else
      throw new IllegalArgumentException( "container type outside legal range" );
  }

  /**
   * ctor: Construct a new temporary variable with a binding and default
   * container type, as well as a linkage for the variable.
   *
   * @param name is the binding.
   * @param ct is the container type, the type of the variable.
   * @param link is the linkage type for the variable.
   * @throws IllegalArgumentException if the container type is outside
   * the legal range [{@link Value#SCALAR}, {@link Value#LIST}], or
   * the linkage is outside [{@link LFN#NONE}, {@link LFN#INOUT}].
   */
  public Local( String name, int ct, int link )
    throws IllegalArgumentException
  {
    super();
    this.m_name = name;
    ...
    if ( LFN.isInRange(link) ) 
      ...
      throw new IllegalArgumentException( "linkage type outside legal range" );
  }
  ...
  /**
   * ctor: Construct a temporary variable with a binding and default
   * value. The container type will be determined from the default
   * value. The linkage is set separately.
   *
   * @param name is the binding.
   * @param value is either a {@link Scalar} or {@link List} value.
   * @param link is the linkage of the value for DAG creation.
   * @throws IllegalArgumentException if the linkage is outside
   * [{@link LFN#NONE}, {@link LFN#INOUT}].
   */
  public Local( String name, Value value, int link )
    throws IllegalArgumentException
  {
    super();
    this.m_name = name;
   ...
   *
   * @return the default as {@link Scalar} or {@link List}, or
  ...
  /**
   * Accessor: Obtains the container type. Note that the container
   * type of the value will be considered first. 
   *
  ...
  {
    return ( m_value != null ) ? 
      m_value.getContainerType() : 
      m_containerType;
  }

  /**
   * Accessor: Establishes a new name for this variable. FIXME: Note
   * that no checks will be done concerning the uniqueness of the new
   * name.
   *
   ...
   * @throws IllegalArgumentException, if the argument is outside
   * the valid range.
   * @see #getLink()
   */
  ...
  /**
   * Accessor: Sets a new container type for the variable. The new
   * container type must match the value's container type.
   *
   * @param containerType is the new integer describing a container type.
   * @throws IllegalArgumentException if the container type is neither
   * {@link Value#SCALAR} nor {@link Value#LIST}.
    ...
    throws IllegalArgumentException
  {
    if ( m_value == null ) {
      ...
      if ( Value.isInRange(containerType) )
	...
	throw new IllegalArgumentException( "new container type does not match container type of default value" );
    }
  }

  /**
   * Accessor: Sets or overwrites the default value of variable.
   * The new default must match the container type.
   *
   * @param value is the new default value. 
   * @throws IllegalArgumentException if the container type of the new
   * value and of the registered container type for the parameter don't
   * match.
   * @see #getValue()
   */
  public void setValue(Value value)
    throws IllegalArgumentException
  { 
    if ( value.getContainerType() == this.m_containerType )
      this.m_value = value;
    else
      // container types do not match
      throw new IllegalArgumentException( "container type of new value does not match Locald container type" );
  }

  /**
   * Dumps the content of the this element into a string for human
   * consumption.
   *
  ...
  public String toString()
  {
    ...
    StringBuffer result =
      new StringBuffer( 12 + this.m_name.length() + value.length() );
  ...
  /**
   * Dumps the content of the variable definition into a string for
   * human consumption.
   *
