...
public class Define extends Directive implements DefinitionHandler
{
   ...
   */
  public Define(DatabaseSchema dbs)
    throws IOException, MissingResourceException
  {
    m_dbschema = dbs;
  }
   ...
   *
   * @return all remembered derivations, an empty set if none
   * were found, or <code>null</code> if remembering was off.
  ...
  /**
   * Insert definitions into database, if a definition already
  ...
  public boolean store( Definition d )
  {
    ...
    if ( m_derivations != null && d instanceof Derivation )
      m_derivations.add( d.shortID() );
	...
	if ( vdc.containsDefinition(d) ) {
	  if ( m_overwrite ) {
	    ...
	    Definition old = vdc.loadDefinition( d.getNamespace(), 
						 d.getName(),
						 d.getVersion(), 
						 d.getType() );
	    old.toXML( m_rejects, "  " );
	...
	m_logger.log( "database", 0, "SQL error " + i + ": " +
