...
/**
 * This class traverses the dependency tree. The tree is built by
 * {@link Derivation} objects. The linkage is data-flow oriented.
...
{
  /**
   * Default max depths of recursion into the graph before the circuit
   * breaker hits.
   */
  public static final int MAXIMUM_DEPTH = 256;

  /**
   * This is a nested class to obscure it from the outside world.
   * It will maintain a stack of database manager backends that 
   * are used to resolve compound transformations. For this purpose,
   * it exposes the required backend database searches, and handles
   * stack descend.<p>
   * The stack enforces that any schemas put onto it implement the
   * <code>VDC</code> interface.
  ...
  class DatabaseSchemaStack
  {
    ...
    /**
     * C'tor: Creates a new stack instance that contains the bottom-most
     * database backend.
     ...
     * @param nextSchema is a new database schema handler, usually an
     * in-memory one for all except the root element. 
     * @exception NullPointerException, if the argument passed is null.
     */
    public void push( DatabaseSchema nextSchema )
	...
	throw new RuntimeException( "illegal database schema: Not a VDC" );
      ...
      Logging.instance().log( "stack", 2, "pushing dbmstack[" +
			      this.m_stack.size() + ']' );
    ...
    /**
     * Descends down the stack of database manager, and for each
     * it will determine the list of elegible logical filenames. The
     * search only searches for Derivations and output filenames. The
     * name to search for is the variable.<p>
     * FIXME: There might be a bug/feature with default args in TRs!
     *
    ...
    public java.util.List derivationsWithOutput( String filename )
    {
      ...
      Logging.instance().log( "trace", 2, "derivationsWithOutput(" +
			      filename + ")" );
      try {
	...
	while ( i.hasPrevious() && flag ) {
	  ...
	  if ( item == null ) {
	    ...
	    Logging.instance().log( "cache", 0, "[" + level + 
				    "] LFN cache MISS for " + 
				    filename );
	    ...
	    // cache hit
	    Logging.instance().log( "cache", 1, "[" + level + 
				    "] LFN cache HIT  for " + 
				    filename );
    ...
    private String genKey( String usesspace, String uses, 
    ...
    /** 
     * Obtain all transformations that exactly match the given secondary
     * key triple and version range. Thus, we have to weed out wildcard
     * matches with exact/range matches, before we can add transformations.
     *
     ...
     */
    public java.util.List searchForTR( String usesspace, String uses,
				       String min, String max )
    {
      java.util.List result = new ArrayList();
      Logging.instance().log( "trace", 2, "searchForTR " +
			      (usesspace==null ? "null" : usesspace) + 
			      Separator.NAMESPACE + uses + Separator.NAME +
			      (min==null ? "null" : min) + Separator.VERSION +
			      (max==null ? "null" : max) );
      try {
	...
	while ( i.hasPrevious() && flag ) {
	  ...
	  if ( item == null ) {
	    // unknown or expired, check database
	    Logging.instance().log( "cache", 0, "[" + level + 
				    "] TR cache MISS for " + key );

	    VDC vdc = (VDC) element.getDatabaseSchema();
	    java.util.List intermediary =
	      vdc.searchDefinition( /* usesspace==null ? "" : */ usesspace, 
				    uses, null, Definition.TRANSFORMATION );
    ...
    /** 
     * Obtain all derivations that wildcard match the given secondary
     * key triple.
     *
     ...
     */
    public java.util.List searchForDV( String namespace, 
				       String name, 
				       String version ) 
    {
      java.util.List result = new ArrayList();
      Logging.instance().log( "trace", 2, "searchForDV " +
			      (namespace==null ? "null" : namespace) + 
			      Separator.NAMESPACE + name + Separator.NAME +
			      (version==null ? "null" : version) );
      try {
	for ( ListIterator i = m_stack.listIterator(m_stack.size());
	      i.hasPrevious(); ) {
	  StackElement element = (StackElement) i.previous();
	  VDC vdc = (VDC) element.getDatabaseSchema();
	  java.util.List list = 
	    vdc.searchDefinition( namespace, name, version,
   ...
   * At the bottom of the stack, queries to the database backend
   * are generated. At higher levels in the stack, the stack-existing
   * definitions take precendence. 
   */
  private DatabaseSchemaStack m_stack;

  /**
   * Stores a reference to the class that manages arbitration in case
   * multiple derivations produce the same file.
   */
  private Arbiter m_arbiter;

  /**
   * The constructor initializes the stack of definitions with the
  ...
  /**
   * The constructor initializes the stack of definitions with the
   * default database manager backend. Additionally, it will push an
   * in-memory schema constructed from the definitions argument onto the
   * stack.
   *
   * @param backend is the backend database manager.
  ...
  /**
   * Creates a new top of stack with the given definitions. Effectively
   * a push() operation. The definitions fragment will be converted
  ...
  private void appendScalarDaxLeaf( Scalar s, java.util.List v )
  {
	...
	(org.griphyn.vdl.classes.Leaf) i.next();
	...
	Logging.instance().log( "default", 0, "WARNING: " +
				"Illegal argument type in scalar or list" );
      }
    }
  }

  /**
   * DAX Bridge: This methods translates a <code>Value</code> tree from
   * a VDL spec into a flattened <code>Leaf</code> tree for a DAX job
   * specification.
   *
   ...
   */
  private java.util.List flattenScalar( org.griphyn.vdl.classes.Value v,
					String prefix,
					String separator,
					String suffix )
  {
	...
	result.add( new PseudoText(prefix) );
	...
	result.add( new PseudoText(suffix) );
      break;
  ...
  /**
   * DAX bridge: Converts a leaf into a mixed form of either the textual
   * string, or the value of the bound variable, as passed from a DV
   * into a TR. The bound variable values will be flattened out.
   *
   ...
   */
  private java.util.List convertLeaf( org.griphyn.vdl.classes.Leaf old,
				      Map arguments )
	   ...
	   IllegalArgumentException
  {
	...
	throw new UndeclaredVariableException( "bound variable \"" +
					       u.getName() +
					       "\" is not known" );
      }
//      java.util.List flat = 
...
//      }
  ...
  /**
   * Finds first TR that matches a DV or Call. If no match is found, an
   * exception will be throws. If more than one TR is found, the first
   * match will be taken - this may not always be the best match.<p>
   *
   ...
   * @exception TransformationNotFoundException will be thrown, if no
   * match is found
   * @exception NullPointerException will be thrown, if uses is null.
   */
  private Transformation findMatchingTransformation( String usesspace,
						     String uses,
						     String minInclude,
						     String maxInclude )
    throws TransformationNotFoundException, NullPointerException
  {
    ...
    String trid = (usesspace == null ? "*" : usesspace ) +
      Separator.NAMESPACE + 
      (uses == null ? "*" : uses) +
      Separator.NAME +
    ...
    java.util.List result =
      this.m_stack.searchForTR( usesspace, uses, minInclude, maxInclude );
    // POSTCONDITION: result contains matching TR

    // select one TR from result set
    int size = result.size();
    Logging.instance().log( "route", 0, size + " matching TR" + 
			    (size==1 ? "" : "s") + " found" );
    if ( size == 0 ) {
  ...
  /** 
   * DAX bridge: Matches <code>Declare</code> (must be complete) with
   * <code>Pass</code> (can skip default args). Use defaults from
   * <code>Use</code>. Flatten out results into a map which unfolds
   * into a vector of DAX leaves, and which are fit for a <code>Job</code>.
   *
   * @param dv is an implementation of <code>HasPass</code>. Implementing
   * classes include <code>Derivation</code> and <code>Call</code>.
   * @param tr is the <code>Transformation</code> which supplies the
   * formal arugments.
   * @return a map from each actual argument or un-overwritten 
   * formal argument default from the argument name to its <code>Value</code>.
   *
   * @exception MissingArgumentException is thrown, if a formal argument
   * was declared without a default, but no actual argument was supplied
   * for it. 
   */
  private Map mapActualToFormal( HasPass dv, Transformation tr )
  {
	...
	Logging.instance().log( "route", 2,
				"adding default value for " + name + '=' +
				farg.getValue().toString() );
      } else {
	// use the supplied actual argument.
	arguments.put( name, aarg.getValue() );
	Logging.instance().log( "route", 2,
				"actual argument taken for " + name + '=' +
  ...
  /**
   * Adds all temporary variables declared <code>Local</code> to
   * the arguments map. Flatten out results into a map which unfolds
   * into a vector of DAX leaves, and which are fit for a <code>Job</code>.
   *
   ...
   *
   * @exception DuplicateIdentifier is thrown, if a bound name already
   * exists in the map. This indicates either a re-used name between 
   * formal args and temporary variables, or the same temporary variable
   * was used twice. 
   */
  private void updateArgsFromLocal( Map arguments, Transformation tr,
				    BookKeeper state )
  {
    // nothing to do
	...
	throw new DuplicateIdentifier( "identifier \"" + name + 
				       "\" already exists" );
	...
	Logging.instance().log( "route", 4, " newfn = " + lfn.getFilename() );
      ...
      Logging.instance().log( "route", 2, 
			      "adding temporary variable " + name + '=' +
			      value.toString() );
    }
  }

  /**
   * DAX bridge: Adds all filenames referenced by args per job to the
   * job "uses" clause note that this is a super-set of filenames, and
   * may exceed the union between profile and CLI filenames.
   *
   * @param job is the job to augment with filenames that are used within.
   * @param arguments are the argument from which LFNs are plucked,
   * and stuck into the "uses" section of the job.
  ...
  private Set augmentJobUses( Job job, Map arguments )
  {
	...
	job.addUses(file);
	Logging.instance().log( "route", 2,
				"adding uses for " + lfn.getFilename() );
      }
    }
    return result;
  }
  ...
  /**
   * DAX bridge: Copies VDL profiles into DAX profiles while
   * substituting any <code>Use</code> elements.
   *
   * @param job is the job to augment with profiles.
   * @param arguments are the arguments from which any <code>Use</code>
   * elements is substituted from. They are just passed through here.
   * @param profiles are all known profiles, from outer to inner TR. 
   */
  private void augmentJobProfile( Job job, Map arguments,
				  java.util.List profiles )
  {
    for ( Iterator i=profiles.iterator(); i.hasNext(); ) {
      org.griphyn.vdl.classes.Profile src =
	(org.griphyn.vdl.classes.Profile) i.next();
	...
	java.util.List n = convertLeaf( (org.griphyn.vdl.classes.Leaf) e.next(),
					arguments );
  ...
  /**
   * DAX bridge: Converts VDL <code>Argument</code> items into DAX
   * arguments while substituting any <code>Use</code> elements againt
   * their value.
   *
   * @param job is the job to augment with profiles.
   * @param arguments are the arguments from which any <code>Use</code>
   * elements is substituted from. They are just passing through here.
   * @param tr is the Transformation that provides the profiles. 
   */
  private void augmentJobArguments( Job job, Map arguments,
				    Transformation tr )
  {
    ...
    for ( Iterator e=tr.iterateArgument(); e.hasNext(); ) {
      org.griphyn.vdl.classes.Argument src =
	(org.griphyn.vdl.classes.Argument) e.next();
	...
	if ( name != null &&
	  ...
	  if ( neu.size() != 1 || ! (neu.get(0) instanceof Filename) ) 
	    throw new IllegalArgumentException( "invalid spec for stdio: You must use exactly one LFN" );
	  else {
   ...
   * LFNs into the necessary usage type. Not all LFNs can be casted.
   *
   * @param ul is the provided linkage in the <code>Use</code> element.
   * @param lfn is the logical filename to adjust. This argument may
   * be adjusted in place, if the cast condition matches
   *
  ...
  {
    if ( lfn.getLink() == LFN.INOUT && (ul == LFN.INPUT || ul == LFN.OUTPUT) )
      lfn.setLink(ul);
  }

  /**
   * Helper function to update the guts of a <code>Scalar</code>, each
   * occurance of a <code>Use</code> by its matching value list from
   * the preset map.
   *
   * @param key is a symbolic representation of the variable that we
   * are currently mapping
   * @param s is the Scalar to remap
   * @param preset is the map with values to replace with.
   * @return the new value from the replacement. This method enforces
   * the return of a Scalar.
   * @see #mapCallToDerivation( String, String, String, Call, Map )
   */
  private Scalar updateScalarFromPreset( String key, Scalar s, 
					 Map preset )
  {
    Logging.instance().log( "route", 5, "updateScalarFromPreset( " + 
			    key + '=' + s.toString() + " )" );
	...
	if ( v2.getContainerType() != Value.SCALAR )
	  throw new RuntimeException( "cannot map list " + use.getName() +
				      " to scalar " + key );
  ...
  /**
   * Helper function to update the guts of a <code>Scalar</code>. 
   * This function just checks for the one very special case in
   * argument passing.
   * <ul>
   ...
   * </ul>
   * If all these conditions are true, the List is returned. 
   * All other cases are handled by {@link #updateScalarFromPreset}.
   *
   * @param key is a symbolic representation of the variable that we
   * are currently mapping
   * @param s is the Scalar to remap
   * @param preset is the map with values to replace with.
   * @return the new value from the replacement. Note that, due to
   * the Use element, the result can be a list to be passed.
   * @see #mapCallToDerivation( String, String, String, Call, Map )
   */
  private Value updateFromPreset( String key, Scalar s, Map preset )
  {
    Logging.instance().log( "route", 5, "updateFromPreset( " + 
			    key + '=' + s.toString() + " )" );
	...
	for ( Iterator i=list.iterateScalar(); i.hasNext(); ) {
	  for ( Iterator j=((Scalar) i.next()).iterateLeaf(); j.hasNext(); ) {
	    org.griphyn.vdl.classes.Leaf l = 
	      (org.griphyn.vdl.classes.Leaf) j.next();
  ...
  /**
   * Maps an anonymous <code>Call</code> to a named <code>Derivation</code>.
   * The <code>Call</code> may contain references to bound variables as
   * <code>Use</code> leaves. These must be translated into their respective
   * actual argument value with the help of the arguments hash.
   *
   * @param namespace is the namespace in which to produce the new Derivation.
   ...
   * @param arguments is a map of actual arguments to substitute for
   * bound references.
   * @return a new <code>Derivation</code> to be used in the stead of the
   * <code>Call</code>.
   */
  private Derivation mapCallToDerivation( String namespace, 
					  String name, String version,
					  Call me, Map arguments )
  {
    Derivation result =
      new Derivation( namespace, name + '.' + me.shortID(), version,
		      me.getUsesspace(), me.getUses(), 
		      me.getMinIncludeVersion(), me.getMaxIncludeVersion() );
    ...
    for ( Iterator f=me.iteratePass(); f.hasNext(); ) {
      ...
      Logging.instance().log( "route", 4, "pold " + key + '=' +
			      pass.getValue().toString() );
	...
	org.griphyn.vdl.classes.List newlist =
	  new org.griphyn.vdl.classes.List();
	Logging.instance().log( "trace", 4, "mapping list to list" );
	for ( int i=0; i<list.getScalarCount(); ++i )
	  newlist.addScalar( updateScalarFromPreset( key + "[" + i + "]",
						     list.getScalar(i),
						     arguments ) );
	value = newlist;
	break;
	...
	throw new RuntimeException( "should not happen" );
      ...
      Logging.instance().log( "route", 4, "pnew " + key + '=' + 
			      value.toString() );
  ...
  /**
   * This private helper methods uses a single derivation or call node
   * and applies the immutable parts stored in the transformation.
   * The result is a job description which merges a call or DV actual
   * argument with transformation formal argument. The result will
   * be stored in the book-keeping DAX.<p>
   *
   * Compound TR will be sequenced into a number of jobs. Each later job
   * will depend on its previous job. The last job in the sequence is
   * the replacement job for the calling derivation. FIXME: what about
   * the parentship of the first job?<p>
   *
   ...
   * @param real are the real derivations to be used as PARENT for the
   * current derivation. In case of a simple TR, the result is the
   * derivation itself.
   * @param level is the recursion depth to record, set by caller
   * @return <code>true</code> for a simple transformation,
   * <code>false</code> for a compound.
   */
  private boolean applyTransformation( Derivation dv, BookKeeper state,
				       TreeSet real, int level )
  {
    // startup
    String id = dv.identify();
    Logging.instance().log( "trace", 3, "applyTransformation(" + id + 
			    ", " + level + ')' );

    // find a TR anywhere on the definitions stack, from top to bottom,
    // that matches the DV-specified constraints.
    Transformation tr =
      findMatchingTransformation( dv.getUsesspace(), dv.getUses(),
    ...
    if ( tr.isSimple() ) {
      ...
      // generate DAX job
      String nmtoken = state.mapJob( id );
      Job job = new Job( tr.getNamespace(), tr.getName(), tr.getVersion(),
			 nmtoken,
			 dv.getNamespace(), dv.getName(), dv.getVersion() );
      job.setLevel(level);
      ...
      if ( profiles.size() > 0 )
	augmentJobProfile( job, arguments, profiles );
	...
	Derivation newdv = 
	  mapCallToDerivation( dv.getNamespace(), dv.getName(), dv.getVersion(),
			       call, arguments );
	Logging.instance().log( "route", 3, "adding DV " + newdv.identify() + 
				" from CALL " + call.identify() );
	...
	if ( (position = defs.positionOfDefinition(newdv)) != -1 ) 
	  Logging.instance().log( "route", 3, "added DV " + 
				  defs.getDefinition(position).identify() );
      }
	...
	Logging.instance().log( "route", 2, "LFN " + lfn + " produced by " + temp.toString() );
	real.addAll(temp);
      }
    ...
    Logging.instance().log( "trace", 2, "applyTransformation(" + id + 
			    ") := " + real.toString() );
    return ( tr.isSimple() );
  }
  ...
  /**
   * This private helper traverses recursively the derivation dependencies,
   * started by querying for a given derivation. FIXME: It is assumed that
   * the derivation is part of the instance-central derivation list.
   *
  ...
  {
    String id = dv.identify();
    Logging.instance().log( "trace", 3, "requestDerivation(" + id + 
			    ", " + level + ')' );
	...
	if ( level <= 1 ) {
	  ...
	  Logging.instance().log( "app", 1, "Skipping direct request for " +
				  "already known DV " + id );
	  result.addAll( state.getVisited(dv) );
	} else {
	  // FIXME: Potential problem, if requested indirectly!
	  final String m = "requestDerivation(" + id + ") reached an unreachable branch. " + 
...
// advantage: breadth-first, correct profiles
	...
	else Logging.instance().log( "route", 4, "NOT adding " + id +
				     " PARENTS " + parents.toString() );
      }
    } else {
      Logging.instance().log( "route", 0, "maximum recursion " + m_maxDepth +
			      " reached, leaving." );
    }

    Logging.instance().log( "trace", 2, "requestDerivation(" + id + 
			    ") := " + result.toString() );
    return result;
  }

  /**
   * This private helper method is used in recursive calls when requesting
   * a given logical filename. 
   *
   * @param lfn is the logical filename that must be a product of some
   * derivation
   ...
   * @return a set of job identifiers
   */
  private Set requestLfn( String lfn, BookKeeper state, int level,
			  String cwns )
  {
    TreeSet result = new TreeSet();
    Logging.instance().log( "trace", 3, "requestLfn(" + lfn + 
			    ", " + level + ", " + 
			    ( cwns == null ? "null" : cwns ) + ')' );
	...
	result.addAll( requestDerivation( dv, state, level+1 ) );
      ...
      if ( level == 0 )
	Logging.instance().log( "default", 0, 
				"Unknown output LFN \"" + lfn + "\"" );
    }

    Logging.instance().log( "trace", 2, "requestLfn(" + lfn +
			    ") := " + result.toString() );
    return result;
  }

  /**
   * This helper method is the entry point when requesting a certain
  ...
  /**
   * This helper method is the entry point when requesting a set of
  ...
  /**
   * Compares two strings, each of which may be null. If both are null,
   * they are considered equal by this function. This function relies on
   * the fact that equals() can deal with null arguments. 
   *
  ...
  /**
   * This helper retrieves a build-DAG for running the specified logical
   * derivation and all its required predecessors. It assumes a fully
   * qualified derivation specification.
   *
   * @param namespace is the namespace the derivation lives in. A value
   * of {@link Separator#DEFAULT} will be assumed for a value of null.
   * @param name is the name of the derivation. Although anonymous
   * derivations are allowed, this method requires a named derivation.
   * @param version is a version string within the minVersion and maxVersion
   * range defined by the derivation, which might be null.
   * @return a book-keeping structure, or null, if either no matching
   * derivation was found.
   */
  public BookKeeper requestDerivation( String namespace,
				       String name,
				       String version )
  {
  ...
  /**
   * This helper retrieves a DAX for running the specified logical
   * derivation match and all its required predecessors. It assumes a fully
   * qualified or partial-null derivation specification.
   *
   * @param namespace is the namespace the derivation lives in. Null is
   * the joker value.
   * @param name is the name of the derivation. Although anonymous
   * derivations are allowed, this method requires a named derivation.
   * @param version is a version string within the minVersion and maxVersion
  ...
  /**
   * This helper retrieves a DAX for a number of symbolically specified
  ...
  /**
   * This method requests a data product logical filename. As a result,
   * the complete build-style DAG for producing the requested filename
   * will be constructed. 
   *
   * @param lfn is the filename that was requested
   * @return the book-keeping information to construct the DAG. Please
   * note that it might be empty, if no derivation is known to produce
   * the file. It will be null, if the definitionslist is empty.
  ...
  /**
   * This method requests multiple data product logical filenames. As a
