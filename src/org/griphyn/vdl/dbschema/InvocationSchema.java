  ...
  implements PTC
{
	   ...
	   SQLException, IOException
  {
    ...
    this.m_dbdriver.insertPreparedStatement( "stmt.save.job",
	"INSERT INTO ptc_job(id,type,start,duration,pid,rusage,stat," +
	"exitcode,exit_msg,args) VALUES (?,?,?,?,?,?,?,?,?,?)" );
    this.m_dbdriver.insertPreparedStatement( "stmt.save.lfn",
	"INSERT INTO ptc_lfn(id,stat,initial,lfn) VALUES (?,?,?,?)" );

    this.m_dbdriver.insertPreparedStatement( "stmt.select.ivr.sk",
   ...
   */
  public long
    saveArchitecture( Architecture arch )
    throws SQLException
  {
    long result = -1;
	...
	Logging.instance().log( "app", 0, "During rusage sequence number: " +
				e.toString().trim() );
	...
	if ( result == -1 ) {
	  Logging.instance().log( "app", 0, "While inserting into rusage: " +
				  e.toString().trim() );
    ...
    throws SQLException
  {
	...
	Logging.instance().log( "app", 0, "During IVR sequence number: " +
				e.toString().trim() );
	throw e; // re-throw
      }
  ...
  /**
   * Splits the canonical FQDN of a definition into its components,
   * and save each component into the database.
   *
   ...
   */
  private int splitDefinition( PreparedStatement ps, String in, int i )
    throws SQLException
  {
    if ( in == null ) {
  ...
  /**
   * Helper function to insert a chunk of the invocation record. This piece
    ...
    throws SQLException
  {
    ...
    try {
      ...
      Logging.instance().log( "app", 0, "During stat sequence number: " +
			      e.toString().trim() );
      Logging.instance().log("xaction", 1, "START rollback" );
      m_dbdriver.rollback();
      Logging.instance().log("xaction", 1, "FINAL rollback" );
    ...
    try {
      ...
      Logging.instance().log( "app", 0, "While inserting into stat: " +
			      e.toString().trim() );
   ...
   * @exception SQLException if something goes awry during insertion.
   */
  protected void saveLFN( long iid, StatCall s )
    throws SQLException
  {
    ...
    try {
      ...
      Logging.instance().log( "app", 0, "While inserting into lfn: " +
			      e.toString().trim() );
      // rollback in safeInvocation()
    ...
    throws SQLException
  {
    ...
    try {
      ...
      Logging.instance().log( "app", 0, "While inserting into job: " +
			      e.toString().trim() );
      // rollback in safeInvocation()
