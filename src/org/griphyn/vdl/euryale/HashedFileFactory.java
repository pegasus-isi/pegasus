...
public class HashedFileFactory extends FlatFileFactory 
{
   ...
   * @param multiplicator is a corrective factor to account for files
   * that are created by the run-time system on the fly. For Euryale and
  ...
  /**
   * Counts the number of times the structured virtual constructor was
   * called.
   * @see #getCount()
   */
  private int m_count;
  ...
  /**
   * Accessor: Obtains the offset for an estimation of total files from
  ...
  /**
   * Accessor: Sets the offset for files which may be created without
    ...
    throws Exception
  {
	...
	def.reset();
	int nr = 1;
	for ( int n=i; n>0; n-- ) nr *= def.getFilesPerDirectory();
	nr -= def.getOffset();
