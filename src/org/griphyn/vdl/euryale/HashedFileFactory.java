 ...
 * which a level change to l occurs.
 *
...
public class HashedFileFactory extends FlatFileFactory {
     ...
     * @param multiplicator is a corrective factor to account for files that are created by the
     *     run-time system on the fly. For Euryale and Pegasus it is safe to assume a multiplicator
     *     of at least 8.
    ...
    /**
     * Number of entries per level. The number 254 is optimized for the Linux VFS ext2, and
     * consequently ext3, which works fastest, if the number of entries per directory, including dot
     * and dotdot, don't exceed 256.
     */
    ...
    /**
     * Offset of files expected to reside at the top level directory. This is counted in addition to
     * the directories being created.
     */
    protected int m_offset = 30;

    /**
     * Helping structure to avoid repeated memory requests. Stores the directory number for each
     * level.
     *
    ...
    /**
     * Helping structure to avoid repeated memory requests. Stores the digits necessary to create
     * one level's directory name.
     *
     * @see #format( int )
     */
    ...
    /**
     * Helping structure to avoid repeated memory requests. Stores the number of digits for
     ...
     *     such.
     */
    public HashedFileFactory(File baseDirectory) throws IOException {
        super(baseDirectory);
        reset();
    }
     ...
     *     such.
     */
    public HashedFileFactory(String baseDirectory) throws IOException {
        super(baseDirectory);
        reset();
    }
     ...
     *     such.
     */
    public HashedFileFactory(File baseDirectory, int totalFiles) throws IOException {
        super(baseDirectory);
        m_levels = calculateLevels(totalFiles, m_multiplicator, m_filesPerDirectory, m_offset);
        reset();
    }
     ...
     *     such.
     */
    public HashedFileFactory(String baseDirectory, int totalFiles) throws IOException {
        super(baseDirectory);
        m_levels = calculateLevels(totalFiles, m_multiplicator, m_filesPerDirectory, m_offset);
        reset();
    }
    ...
    /**
     * Creates the next file with the given basename. This is the factory standard virtual
     * constructor. Once invoked, the directory structure can not be changed any more.
     *
     * @param basename is the filename to create. Don't specify dirs here.
    ...
    /**
     * Returns the number of times the regular virtual constructor for structured entries was
     * called.
     *
    ...
    /**
     * Accessor: Sets the number of directory levels. Note that this modificator can only be called
     * before the virtual constructor is called the first time.
     *
     ...
     *
     * @return the chosen number of entries per directory excluding the dot and dotdot files.
     */
     ...
     * For a Linux ext2 and thus ext3 system, the optimal maximum number is 254.
     *
    ...
    /**
     * Accessor: Obtains the multiplicative factor for an estimation of total files from calls to
    ...
    /**
     * Accessor: Sets the multiplicative factor to account for files which may be created without
    ...
    /**
     * Accessor: Obtains the offset for an estimation of total files from calls to the virtual
     * constructor.
     *
     * @return the offset
     * @see #setOffset(int)
    ...
    /**
     * Accessor: Sets the offset for files which may be created without calling the virtual
     * constructor.
     *
            ...
            for (int i = 1; i < 4; ++i) {
                ...
                for (int j = -2; j < Integer.MAX_VALUE; ++j) {
                    ...
                    if (level < def.getLevels()) {
                        ...
                        System.out.println(
                                "mult="
                                        + def.getMultiplicator()
                                        + ", offset="
                                        + def.getOffset()
                                        + ", fpd="
                                        + def.getFilesPerDirectory()
                                        + ": nr="
                                        + n
                                        + " => l="
                                        + level);
