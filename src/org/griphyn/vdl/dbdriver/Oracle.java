 ...
 * currently an empty non-working stand-in. We will fill this stand-in with life at some later time.
 *
 * <p>In order to use the Oracle driver, you must have access to Oracle's thin JDCB client.
 *
 * <p>
 *
...
public class Oracle extends DatabaseDriver {
    ...
    public boolean connect(String url, Properties info, Set tables)
        ...
        return this.connect("org.postgresql.Driver", url, info, tables);
    }
    ...
    public void driverMatch() throws SQLException {
        ...
        if (!db.substring(0, jdbc.length()).equals(jdbc))
            throw new RuntimeException(
                    "JDBC driver version " + jdbc + " does not match DBMS version " + db);
    }
