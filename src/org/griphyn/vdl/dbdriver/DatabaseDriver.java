...
/**
 * This common database interface that defines basic functionalities
 ...
 * a functionality that is independent of the schemas to be used.<p>
 * The schema classes implement all their database access in terms of
 * this database driver classes.<p>
...
public abstract class DatabaseDriver
{
  ...
  /**
   * This list stores the prepared statements by their position.
  ...
  /**
   * Instantiates the appropriate child according to property values.
   ...
   * The passed value should not be <code>null</code>.
	   ...
	   SQLException
  {
    ...
    log.log( "dbdriver", 3, "accessing loadDriver( " +
    ...
    if ( dbDriverName == null ) {
      dbDriverName = ChimeraProperties.instance()
	.getDatabaseDriverName( propertyPrefix );
      if ( dbDriverName == null )
    ...
    // done
    if ( result == null )
      log.log( "dbdriver", 0, "unable to load " + dbDriverName );
    else
      log.log( "dbdriver", 3, "successfully loaded " + dbDriverName );
    return result;
  }
   ...
   */
  static public DatabaseDriver
    loadDriver( String propertyPrefix )
    throws ClassNotFoundException, IOException,
	   NoSuchMethodException, InstantiationException,
	   IllegalAccessException, InvocationTargetException,
	   SQLException
  ...
  /**
   * Establishes a connection to the specified database. The parameters
   ...
   * mode, and explicit commits must be issued.<p>
   ...
   */
  protected boolean connect( String driver,
			     String url,
			     Properties info,
			     Set tables )
   ...
   * @exception if the driver is incapable of establishing a connection.
   */
  abstract public boolean connect( String url,
				   Properties info,
				   Set tables )
    throws SQLException, ClassNotFoundException;
  ...
  /**
   * Closes an open connection to the database whenever this object
   ...
   */
  abstract public boolean cachingMakesSense();
  ...
  /**
   * Rolls back the latest changes to the database. Some databases may
  ...
  /**
   * Clears all warnings reported for this database driver. After a call
   ...
   */
  abstract public long sequence1( String name )
    throws SQLException;
   ...
   * @exception if something goes wrong while fetching the new value.
   */
  abstract public long sequence2( Statement s, String name, int pos )
    throws SQLException;

  /**
   * Removes all rows that match the provided keyset from a table.
   * @param table is the name of the table to remove values from
   * @param columns is a set of column names and their associated
   *                values to select the removed columns. The map
   *                may be null to remove all rows in a table.
    ...
    throws SQLException
  {
    ...
    Logging.instance().log( "xaction", 1, "FINAL DELETE in " + table +
			    ": " + count );
   ...
   * @param keycolumns is a set of primary keys and their associated values.
   *                   For special tables, the primary key set may be null
   *                   or empty (e.g. a table without primary keys).
    ...
    throws SQLException
  {
    ...
    Logging.instance().log( "xaction", 1, "FINAL INSERT in " + table +
			    ": " + count );
    return count;
  }
    ...
    throws SQLException
  {
    ...
    int count = m_connection.createStatement().executeUpdate(query);
    Logging.instance().log( "xaction", 1, "FINAL UPDATE in " + table +
			    ": " + count );
    return count;
  }

  /**
   * Selects any rows in one or more colums from one or more tables
   ...
   */
  public ResultSet select( List select,
			   String table,
			   Map where,
			   String order )
    throws SQLException
   ...
   * @param operator is a collection of column names and operators
   *        if no entry is found for the name, then use '=' as default
   * @param order is an optional ordering string.
   */
  public ResultSet select( List select,
			   String table,
  ...
  /**
   * Drills a hole into the nice database driver abstraction to the JDBC3
   ...
   *
   */
  abstract public boolean preferString();

  /**
   * Inserts a new prepared statement into the list of prepared
   ...
   * @return true, if the statement was parsed and added at the id,
   * false, if the id was already taken. The statement is not parsed
   * in that case.
   *
   ...
   */
  public boolean addPreparedStatement( String id,
				       String statement )
    throws SQLException
  {
    // sanity check
    ...
    // duplicate check
    if ( this.m_prepared.containsKey(id) )
      return false;
	...
	Logging.instance().log( "sql", 0, "SQL error " + i + ": " +
   ...
   * @param id is the id into which to parse the statement
   * @param statement is the before-parsing statement string
   * @param autoGeneratedKeys is true, if the statement should reserve
   * space to return autoinc columns, false, if the statement does not
   * have any such keys.
   * @return true, if the statement was parsed and added at the id,
   * false, if the id was already taken. The statement is not parsed
   * in that case.
   ...
   * @see #cancelPreparedStatement( String )
   */
  protected boolean addPreparedStatement( String id,
					  String statement,
    ...
    throws SQLException
  {
    ...
    try {
      this.m_prepared.put( id,
        this.m_connection.prepareStatement( statement,
                                            autoGeneratedKeys ?
  ...
  /**
   * Inserts a new prepared statement into the list of prepared
   * statements. If the id is already taken, an error will be
   * printed and execution aborted.
   *
   * @param id is the id into which to parse the statement
   * @param statement is the before-parsing statement string
   * @return true, if the statement was parsed and added at the id,
   ...
   * @see #cancelPreparedStatement( String )
   */
  public boolean insertPreparedStatement( String id,
					  String statement )
    throws SQLException
  {
  ...
  /**
   * Obtains a reference to a prepared statement to be used from
   * the caller. This function will also reset the input values
  ...
  /**
   * Explicitely requests a prepared id to be destroyed and its resources
   * freed. Multiple invocation for the same id are harmless.
   *
   * @param id is the place of the statement to free up.
   * @exception SQLException if the database does not like the disconnect.
   *
   ...
   * @see #cancelPreparedStatement( String )
   */
  public void removePreparedStatement( String id )
    throws SQLException
  {
   ...
   *
   * @param id is the id for which to obtain the previously
   * prepared statement.
   *
   * @exception SQLException if a database access error occurs while
   * clearing the parameters.
   * @exception ArrayIndexOutOfBoundsException if a non-existing id
   * is being requested.
   *
   * @see java.sql.PreparedStatement#clearParameters()
   * @see #addPreparedStatement( String, String )
