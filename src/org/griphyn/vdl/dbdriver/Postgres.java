...
/**
 * This class implements the driver API for the PostGreSQL 7.3.* and
...
{
  /**
   * Caches the driver version major, because this has side-effects on
   * the behavior.
   */
    ...
    throws SQLException
  {
	...
	throw new RuntimeException(
	  "JDBC driver " + jdbc +
	  " does not match database version " + database );
    } else {
  ...
  /**
   * Obtains the next value from a sequence. Postgres uses explicit
   * sequence generators, so this function will return the new id.
   *
    ...
    throws SQLException
  {
    ...
    rs.close();
    Logging.instance().log("xaction", 1, "FINAL sequence " + name + 
			   " = " + result );
  ...
  /**
   * Obtains the sequence value for the current statement. Postgres does
   * not permit NULL-driven auto-increment columns. Postgres uses
   * explicit sequence generators, so this function always returns -1.
   *
