...
public class Chain extends Default {
     ...
     * site. If the bundle value is not specified, then null is stored.
     */
    private Map mSIBundleMap;


    /**
     * A map indexed by execution sites. Each value is a SiteTransfer object,
     * that contains the Bundles of stagin transfer jobs.
     *
     * @see TransferChain
     */
...
//        mDAG.addNewRelation(parent,child);
    ...
    public void done(){
        ...
        for(Iterator it = mStageInMap.entrySet().iterator();it.hasNext();){
            ...
            mLogger.log(entry.getKey() + " " + entry.getValue(),
                        LogManager.DEBUG_MESSAGE_LEVEL);
        }
    }
     ...
     * single site. The container maintains the bundles and controls the
    ...
    private class SiteTransfer{
         ...
         * particular site. This should correspond to the bundle factor.
         */
        ...
        /**
         * The list of <code>Chain</code> object. Each bundle is actually a chain
         * of transfer nodes.
         */
         ...
         * @param bundle  the number of logical bundles that are to be created
         *                per site. it directly translates to the number of transfer
         *                jobs that can be running at a particular site
         */
         ...
         *
         * @return  the last transfer job in the chain before the current job
         *          was added, null in case the job is the first in the chain
         */
