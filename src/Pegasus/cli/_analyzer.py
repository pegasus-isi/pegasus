"""pegasus analyzer — report successful and failed jobs."""
...
@click.command("analyzer", cls=HelpCmd, options_metavar="<options>")
@click.pass_context
@click.option(
...
@click.option(
    ...
    type=str,
    metavar="DEBUG_JOB",
    help="Specifies a job to debug (can be either the job base name or the submit file name)",
)
@click.option(
    "--local-executable",
...
def analyzer_cmd(
    ...
    if json_mode:
        ...
        console_output = _generate_analyzer_output(ctx, options, output)
    print(console_output)
    print()

    if ai:
        print(_indent_console(" Pegasus AI Analysis ".center(80, "*")))
        agent_output = None
        try:
...
def _generate_analyzer_output(ctx, options, output):
    cout = ""
    for wf in output.workflows:
        cout += _generate_output(ctx, options, output.workflows[wf])
...
def _generate_output(ctx, options, wf):
    cout = ""
    counts = wf.jobs
    ...
    cout += "\n"
    cout += _indent_console(" Summary ".center(80, "*"))
    cout += "\n"
    cout += _indent_console(" Submit Directory   : %s" % (wf.submit_dir))
    cout += _indent_console(" Workflow Status    : %s" % (wf.wf_status))

    cout += _indent_console(" Total jobs         : % 6d" % (counts.total))
    cout += _indent_console(
...
def _generate_held_jobs(options, held_jobs):
    cout = ""
    cout += _indent_console(" Held jobs' details ".center(80, "*"))
    cout += "\n"

    for job in held_jobs:
        held_job = held_jobs[job]
        cout += _indent_console(job.center(80, "="))
        cout += "\n"
        cout += _indent_console(
        ...
        cout += _indent_console("reason                 : %s" % (held_job["reason"]))
        cout += "\n"

    return cout


def _generate_failing_jobs(options, failing_jobs):
    cout = ""
    cout += _indent_console("failing jobs' details".center(80, "*"))
    for my_job in failing_jobs:
        _cout, _ = _generate_job_instance(options, failing_jobs[my_job])
        cout += _cout
    return cout


def _generate_failed_jobs(options, failed_jobs):
    cout = ""
    cout += _indent_console("Failed jobs' details".center(80, "*"))

    for my_job in failed_jobs:
        sout, sub_wf_cmd = _generate_job_instance(options, failed_jobs[my_job])
        ...
        if sub_wf_cmd is not None and options.recurse_mode:
            ...
            cout += _indent_console(("").center(80, "="))
        else:
            cout += sout
...
def _generate_unknown_jobs(options, unknown_jobs):
    cout = ""
    cout += _indent_console("Unknown jobs' details".center(80, "*"))

    for my_job in unknown_jobs:
        _cout, _ = _generate_job_instance(options, unknown_jobs[my_job])
        cout += _cout

    return cout


def _indent_console(stmt=""):
    return indent + stmt + "\n"


def _generate_job_instance(options, job_instance_info):
    cout = ""
    sub_wf_cmd = None

    cout += "\n"
    cout += _indent_console(job_instance_info.job_name.center(80, "="))
    cout += "\n"
    cout += _indent_console(" last state: %s" % (job_instance_info.state or "-"))
    cout += _indent_console("       site: %s" % (job_instance_info.site or "-"))
    cout += _indent_console("submit file: %s" % (job_instance_info.submit_file or "-"))
    cout += _indent_console("output file: %s" % (job_instance_info.stdout_file or "-"))
    cout += _indent_console(" error file: %s" % (job_instance_info.stderr_file or "-"))
    ...
    if options.print_pre_script and len(job_instance_info.pre_executable or "") > 0:
        cout += "\n"
        cout += _indent_console("SCRIPT PRE:")
        cout += _indent_console(
            "%s %s"
            % (
    ...
    if job_instance_info.subwf_dir != "-":
        ...
        if not options.recurse_mode:
            cout += _indent_console(" This job contains sub workflows!")
            cout += _indent_console(
                " Please run the command below for more information:"
            )
            cout += _indent_console(sub_wf_cmd)
        cout += "\n"
    cout += "\n"

    cout += _generate_tasks_info(options, job_instance_info)

    return cout, sub_wf_cmd


def _generate_tasks_info(options, job_instance_info):
    cout = ""
    ...
    for task in job_tasks:
        ...
        cout += "\n"
        cout += _indent_console("site        : %s" % (job_instance_info.site))
        cout += _indent_console("hostname    : %s" % (job_instance_info.hostname))
        cout += _indent_console("executable  : %s" % (my_task.executable))
        cout += _indent_console("arguments   : %s" % (my_task.arguments))
        cout += _indent_console("exitcode    : %s" % (my_task.exitcode))
        cout += _indent_console("working dir : %s" % (job_instance_info.work_dir))
        cout += "\n"
        ...
        if not options.quiet_mode:
            ...
            if options.use_files and ji_stdout_text != "-":
                if ji_stdout_text:
                    ...
                    cout += _indent_console(stdout_output)
                    cout += _indent_console(ji_stdout_text)
            else:
    ...
    if ji_stderr_text and ji_stderr_text.strip("\n\t \r") != "" and my_print_job_stderr:
        ...
        cout += "\n"
        cout += _indent_console(ji_stderr_text)
        cout += "\n"
