"""pegasus statistics — generate workflow execution statistics."""
...
@click.command("statistics", context_settings=dict(help_option_names=["-h", "--help"]))
@click.option(
    "-o",
    "--output",
    "output_dir",
...
@click.option(
    ...
    help="Select output file type. Valid values are 'text' and 'csv'.",
)
@click.option(
    "-c",
    "--conf",
...
@click.option(
    ...
    default="all",
    callback=_arg_split,
    help="Comma separated list. Valid levels are: all,summary,wf_stats,jb_stats,tf_stats,ti_stats,int_stats.",
)
@click.option(
    "-t",
    "--time-filter",
    ...
    help="Valid levels are: day,hour.",
)
@click.option(
    "-i",
