...
if __name__ == "__main__":
    ...
    # Use PEGASUS_BIN_DIR (set by Condor.java in job environments) or
    # PEGASUS_HOME/bin, falling back to PATH search via find_prog.
    extra_dirs = []
    pegasus_bin_dir = os.getenv("PEGASUS_BIN_DIR")
    if pegasus_bin_dir:
        extra_dirs.append(pegasus_bin_dir)
    pegasus_home = os.getenv("PEGASUS_HOME")
    if pegasus_home:
        extra_dirs.append(pegasus_home + "/bin")
    monitord_bin = find_prog("pegasus-monitord", extra_dirs)
