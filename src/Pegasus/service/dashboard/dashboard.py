            ...
            for t in workflow.get_transformation_statistics():
                content.append(
                    [
                        ...
                        f"{t.avg:.2f}",
                        f"{t.sum:.2f}",
                        f"{t.min_maxrss / 1024:.2f}" if t.min_maxrss else "-",
                        f"{t.max_maxrss / 1024:.2f}" if t.max_maxrss else "-",
                        f"{t.avg_maxrss / 1024:.2f}" if t.avg_maxrss else "-",
                        f"{t.min_avg_cpu * 100:.2f}%" if t.min_avg_cpu else "-",
                        f"{t.max_avg_cpu * 100:.2f}%" if t.max_avg_cpu else "-",
                        f"{t.avg_avg_cpu * 100:.2f}%" if t.avg_avg_cpu else "-",
                    ]
                )
