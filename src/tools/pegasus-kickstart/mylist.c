...
int mylist_item_init(mylist_item_p item, const char* data) {
...
int mylist_item_done(mylist_item_p item) {
     ...
     */
    /* sanity check */
    if (item == NULL || item->magic != ITEM_MAGIC) {
        return EINVAL;
    }
...
int mylist_add(mylist_p list, const char* data) {
    int status;
    mylist_item_p temp;

    /* sanity check */
    if (list == NULL || list->magic != LIST_MAGIC) {
        return EINVAL;
    }

    /* allocate item space */
    if ((temp = malloc(sizeof(mylist_item_t))) == NULL) {
        return ENOMEM;
    }
...
int mylist_fill(mylist_p list, const char* fn) {
    ...
    while (fgets(line, size, file)) {
        ...
        if ((result = mylist_add(list, line)) != 0) {
            break;
        }
    }
