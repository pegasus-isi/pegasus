 ...
 * system properties. This class solely exists to check the currently
 * active property values.
 */
public class ShowProperties {

  /**
   * Runs the main program. If no arguments were specified, all property
   * values, sorted by their key, will be shown. Otherwise, only property
   * values of matching keys are shown. 
   *
   * @param args are the commandline interface arguments. 
   */
  public static void main( String[] args )
  {
	...
	System.out.println( sp.getProperty(args[i]) );
      }
    } else {
      // all components
      for ( Iterator i=(new TreeSet(sp.keySet())).iterator(); i.hasNext(); ) {
	String key = (String) i.next();
	System.out.println( key + '=' + ((String) sp.get(key)) );
      }
    }
  }
}
