...
#
cp=`( find $PEGASUS_HOME/lib -perm -0500 -name '*.jar' | grep -v gvds.jar | \
    tr '\012' ':' ; echo "" ) | sed -e 's/::/:/g' -e 's/^://' -e 's/:$//'`
if [ "X${CLASSPATH}" != "X" ]; then
    cp=`(( echo $CLASSPATH ; echo $cp ) | tr ':' '\012' | grep -v gvds.jar | \
	grep -v ${PEGASUS_HOME}/build/classes | grep -v ${PEGASUS_HOME}/dist | \
	sort -u | tr '\012' ':' ; echo "" ) | sed -e 's/::/:/g' -e 's/^://' -e 's/:$//'`
fi
CLASSPATH=$PEGASUS_HOME/build/classes:$cp
export CLASSPATH
unset cp
