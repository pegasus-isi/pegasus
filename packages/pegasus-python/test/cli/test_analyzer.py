...
class TestAnalyzeDB:
    ...
    def test_prescript_failure_with_print_invocation(
        ...
        analyze = AnalyzerDatabase(
            Options(input_dir=submit_dir, print_invocation=True, print_pre_script=True)
        )
    ...
    def test_analyze_db_for_wf_failed_wf_zero_failed_jobs(
        self, mocker, capsys, AnalyzerDatabase
    ):
        mocker.patch(
            "Pegasus.analyzer.AnalyzeDB.get_workflow_status",
    ...
    def test_analyze_db_for_wf_failing_jobs(self, mocker, capsys, AnalyzerDatabase):
        ...
        AnalyzeDB.print_job_instance.assert_called_once_with(
            "job-id-0", "job-instance-0", None
        )
...
class TestAnalyzeFiles:
    ...
    def test_analyze_files_monitord(self, mocker, capsys, AnalyzerFiles):
        ...
        analyze = AnalyzerFiles(Options(run_monitord=True, input_dir=submit_dir))
        with pytest.raises(Exception) as err:
            analyze.analyze_files()
        assert "One or more workflows failed" in str(err)
    ...
    def test_print_job_info_subdag_job(self, mocker, capsys, AnalyzerFiles):
        ...
        analyze = AnalyzerFiles(
            Options(output_dir="/random/dir", print_pre_script=True)
        )
    ...
    def test_print_job_info_subdag_job2(
        self, mocker, capsys, AnalyzerFiles, recurse_mode, expected_output
    ):
    ...
    def test_print_output_error(mocker, capsys, AnalyzerFiles):
        ...
        job.out_file = os.path.join(
            directory,
            "analyzer_samples_dir/process_wf_failure/00/00/ls_ID0000001.out.001",
        )
        job.err_file = os.path.join(
            directory,
            "analyzer_samples_dir/process_wf_failure/00/00/ls_ID0000001.err.001",
        )
