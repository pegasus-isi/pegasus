...
from Pegasus.service.base import ErrorResponse, OrderedDict, OrderedSet, PagedResponse
...
class PegasusJsonEncoder(JSONEncoder):
    ...
    def default(self, obj):
        ...
        if isinstance(obj, PagedResponse):
            ...
            json_record = OrderedDict([("code", obj.code), ("message", obj.message)])

            if obj.errors:
                json_record["errors"] = [
                    {"field": f, "errors": e} for f, e in obj.errors
                ]
            ...
            json_record["_links"] = OrderedDict(
                [
                    (
                        "workflow",
                        url_for(".get_workflows", m_wf_id=obj.wf_id, _method="GET"),
                    )
                ]
            )
            ...
            json_record["_links"] = OrderedDict(
                [
                    (
                        "workflow_meta",
                        url_for(".get_workflow_meta", wf_id=obj.wf_id, _method="GET"),
                    ),
                    (
                        "workflow_state",
                        url_for(".get_workflow_state", wf_id=obj.wf_id, _method="GET"),
                    ),
                    (
                        "job",
                        url_for(".get_workflow_jobs", wf_id=obj.wf_id, _method="GET"),
                    ),
                    (
                        "task",
                        url_for(".get_workflow_tasks", wf_id=obj.wf_id, _method="GET"),
                    ),
                    (
                        "host",
                        url_for(".get_workflow_hosts", wf_id=obj.wf_id, _method="GET"),
                    ),
                    (
                        "invocation",
                        url_for(
                            ".get_workflow_invocations", wf_id=obj.wf_id, _method="GET"
                        ),
                    ),
                ]
            )
            ...
            json_record["_links"] = OrderedDict(
                [
                    (
                        "job_instance",
                        url_for(
                            ".get_job_instance", job_instance_id=obj.job_instance_id
                        ),
                    )
                ]
            )
            ...
            json_record["_links"]["host"] = (
                url_for(".get_host", host_id=obj.host_id) if obj.host_id else None
            )
