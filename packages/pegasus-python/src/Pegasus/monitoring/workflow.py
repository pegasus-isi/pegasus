...
UNKNOWN_FAILURE_CODE = 2  # unknown failure code when inserting an END event between consecutive workflow start events
...
class Workflow:
    ...
    def parse_in_file(self, job, tasks):
        ...
        for line in IN:
            ...
            if line.startswith("#@"):
                ...
                # Or it can be the EDGES described in input file for pegasus-mpi-cluster
                continue
            else:
    ...
    def increment_job_counter(self, jobid):
        ...
        if jobid in self._job_counters:
            # Counter already exists for this job, just increase it by 1
    ...
    def job_update_info(self, jobid, job_submit_seq, sched_id=None):
        """
        This function adds info to an existing job.
        """
    ...
    def update_job_state(
        ...
        if self._sink is None and not self._enable_notifications:
            # Not generating events and notifications, nothing else to do
            return
