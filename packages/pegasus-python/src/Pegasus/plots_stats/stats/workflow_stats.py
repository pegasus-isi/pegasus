...
def print_workflow_summary(workflow_stats):
    ...
    summary_str += NEW_LINE_STR
    summary_str += "total_job_instance_retries:" + convert_to_str(
        total_job_instance_retries
    )
    summary_str += NEW_LINE_STR
    ...
    summary_str += NEW_LINE_STR
    summary_str += "total_unsubmitted_sub_wfs: " + convert_to_str(
    ...
    if workflow_wall_time is None:
        ...
        summary_str += "workflow_runtime: %-20s (total %d seconds)" % (
            format_seconds(workflow_wall_time),
            (workflow_wall_time),
        )
    summary_str += NEW_LINE_STR
    ...
    if workflow_cum_job_wall_time is None:
        ...
        summary_str += (
            "cumulative_workflow_runtime_kickstart: %-20s (total %d seconds)"
    ...
    if submit_side_job_wall_time is None:
        ...
        summary_str += (
            "cumulative_workflow_runtime_dagman: %-20s (total %d seconds)"
...
def print_individual_workflow_stats(workflow_stats, title):
    ...
    content = [
        "&nbsp;",
        "Tasks",
        convert_to_str(total_succeeded_tasks),
        convert_to_str(total_failed_tasks),
        convert_to_str(total_unsubmitted_tasks),
        convert_to_str(total_tasks),
        "&nbsp;",
        convert_to_str(total_task_retries),
        convert_to_str(total_task_invocations),
        "&nbsp;",
    ]
    ...
    content = [
        "&nbsp;",
        "Jobs",
        convert_to_str(total_succeeded_jobs),
        convert_to_str(total_failed_jobs),
        convert_to_str(total_unsubmitted_jobs),
        convert_to_str(total_jobs),
        "&nbsp;",
        convert_to_str(total_job_retries),
        convert_to_str(total_job_invocations),
        "&nbsp;",
    ]
    ...
    content = [
        "&nbsp;",
        "Sub Workflows",
        convert_to_str(total_succeeded_sub_wfs),
        convert_to_str(total_failed_sub_wfs),
        convert_to_str(total_unsubmitted_sub_wfs),
        convert_to_str(total_sub_wfs),
        "&nbsp;",
        convert_to_str(total_sub_wfs_retries),
...
def print_wf_transformation_stats(workflow_stats, title):
    ...
    for transformation in workflow_stats.get_transformation_statistics():
        content = [
            ...
            str(transformation.success),
            str(transformation.failure),
            round_to_str(transformation.min),
            round_to_str(transformation.max),
            round_to_str(transformation.avg),
            round_to_str(transformation.sum),
        ]
...
def print_statistics_by_time_and_host(workflow_stats, time_filter):
    ...
    statistics_by_time_str += (
        "<div>#Job instances statistics per " + time_filter + "</div>"
    )
    statistics_by_time_str += NEW_LINE_STR
    ...
    statistics_by_time_str += NEW_LINE_STR
    statistics_by_time_str += (
        "<div>#Invocation statistics run per " + time_filter + "</div>"
    )
    statistics_by_time_str += NEW_LINE_STR
    statistics_by_time_str += "<table class ='gallery_table'>"
    statistics_by_time_str += print_row(time_stats_col_name, True)
    ...
    statistics_by_time_str += NEW_LINE_STR
    statistics_by_time_str += (
        "<div>#Job instances statistics on host per " + time_filter + "</div>"
    )
    statistics_by_time_str += NEW_LINE_STR
    statistics_by_time_str += "<table class ='gallery_table'>"
    ...
    statistics_by_time_str += NEW_LINE_STR
    statistics_by_time_str += (
        "<div>#Invocation statistics on host per " + time_filter + "</div>"
    )
    statistics_by_time_str += NEW_LINE_STR
    statistics_by_time_str += "<table class ='gallery_table'>"
    statistics_by_time_str += print_row(time_host_stats_col_name, True)
