...
class Version(BaseVersion):
    ...
    def update(self, force=False):
        ...
        try:
            ...
            self.db.execute(text("CREATE TABLE rc_lfn_new ( LIKE rc_lfn )"))
            self.db.execute(text("ALTER TABLE rc_lfn_new ADD COLUMN site VARCHAR(245)"))
            log.debug("    Removing index...")
            self.db.execute(text("ALTER TABLE rc_lfn_new DROP INDEX sk_rc_lfn"))
            ...
            self.db.execute(
                text("INSERT INTO rc_lfn_new(id, lfn, pfn) SELECT * FROM rc_lfn")
            )
            ...
            self.db.execute(text("DELETE FROM rc_attr WHERE name='pool'"))
            ...
            data = self.db.execute(
                text("SELECT COUNT(id) FROM rc_attr WHERE name='pool'")
            ).first()
            if data is not None:
                if data[0] > 0:
                    self.db.rollback()
    ...
    def downgrade(self, force=False):
        try:
            ...
            self.db.execute(text("CREATE TABLE rc_lfn_new ( LIKE rc_lfn )"))
            self.db.execute(text("ALTER TABLE rc_lfn_new DROP INDEX sk_rc_lfn"))
            self.db.execute(text("ALTER TABLE rc_lfn_new DROP COLUMN site"))
            log.debug("    Adding new constraint...")
            self.db.execute(
                text("ALTER TABLE rc_lfn_new ADD CONSTRAINT sk_rc_lfn UNIQUE(lfn,pfn)")
            )
            log.debug("    Copying data...")
            self.db.execute(
                text(
