...
class ManifestListener(threading.Thread):
    def __init__(self, url, sliceid, manifest_exchange="testManifestExchange"):
        ...
        self.channel.exchange_declare(
            exchange=self.manifest_exchange, exchange_type="topic"
        )
        ...
        self.channel.queue_bind(
            exchange=self.manifest_exchange,
    ...
    def manifest_message(self, ch, method, properties, body):
        ...
        log.info(
            "Slice Manifest: sliceStatus = %s, numWorkersReady = %d",
            self.status,
            self.current,
        )
    ...
    def run(self):
        self.channel.basic_consume(
            self.manifest_message, queue=self.manifest_queue, no_ack=True
        )
...
class RequestPublisher:
    ...
    def send_modify_request(self, deadline, deadline_diff, util_max, current, required):
        ...
        self.channel.basic_publish(
            exchange="", routing_key=self.request_queue, body=json.dumps(req)
        )
        self.lock.release()
