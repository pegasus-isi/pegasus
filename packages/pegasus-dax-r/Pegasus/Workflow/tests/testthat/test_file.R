...
f1 <- AddProfile(f1, Profile(Pegasus.Namespace$ENV, 'PATH', '/bin'))
f1 <- AddProfile(f1, Profile(Pegasus.Namespace$CONDOR, 'universe', 'vanilla'))
...
test_that('adding profiles', {
  expect_error(AddProfile(f1, Profile(Pegasus.Namespace$ENV, 'PATH', '/bin')), 'Duplicate profile: /bin')
  expect_error(AddProfile(f1, Profile(Pegasus.Namespace$CONDOR, 'universe', 'vanilla')), 'Duplicate profile: vanilla')
  expect_equal(length(f1$catalog.type$profile.mixin$profiles), 2)
  expect_is(f1$catalog.type$profile.mixin$profiles[[1]], 'Profile')
  expect_match(f1$catalog.type$profile.mixin$profiles[[1]]$namespace, Pegasus.Namespace$ENV)
