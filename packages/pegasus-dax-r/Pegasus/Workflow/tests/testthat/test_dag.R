...
dag1 <- AddProfile(dag1, Profile(Pegasus.Namespace$ENV, 'PATH', '/bin'))
dag1 <- AddProfile(dag1, Profile(Pegasus.Namespace$CONDOR, 'universe', 'vanilla'))
...
test_that('adding profiles', {
  expect_error(AddProfile(dag1, Profile(Pegasus.Namespace$ENV, 'PATH', '/bin')), 'Duplicate profile: /bin')
  expect_error(AddProfile(dag1, Profile(Pegasus.Namespace$CONDOR, 'universe', 'vanilla')), 'Duplicate profile: vanilla')
  ...
  expect_match(dag1$abstract.job$profile.mixin$profiles[[1]]$namespace, Pegasus.Namespace$ENV)
...
test_that('adding invokes', {
  ...
  expect_match(dag1$abstract.job$invoke.mixin$invocations[[1]]$when, Pegasus.When$AT_END)
