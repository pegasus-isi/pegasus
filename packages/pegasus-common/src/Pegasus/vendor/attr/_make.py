...
class _ClassBuilder:
    ...
    def add_eq(self):
        ...
        cd["__eq__"], cd["__ne__"] = (
            self._add_method_dunders(meth) for meth in _make_eq(self._cls, self._attrs)
        )
...
def _attrs_to_init_script(
    ...
    if frozen is True:
        if slots is True:
            ...
            def fmt_setter(attr_name, value_var):
                return "_setattr('{attr_name}', {value_var})".format(
                    attr_name=attr_name, value_var=value_var,
                )

            def fmt_setter_with_converter(attr_name, value_var):
                conv_name = _init_converter_pat.format(attr_name)
                return "_setattr('{attr_name}', {conv}({value_var}))".format(
                    attr_name=attr_name, value_var=value_var, conv=conv_name,
                )
            ...
            def fmt_setter(attr_name, value_var):
                if _is_slot_attr(attr_name, base_attr_map):
                    res = "_setattr('{attr_name}', {value_var})".format(
                        attr_name=attr_name, value_var=value_var,
                    )
                else:
                    res = "_inst_dict['{attr_name}'] = {value_var}".format(
                        attr_name=attr_name, value_var=value_var,
                    )
                return res
        ...
        def fmt_setter(attr_name, value):
            return "self.{attr_name} = {value}".format(
                attr_name=attr_name, value=value,
            )

        def fmt_setter_with_converter(attr_name, value_var):
            conv_name = _init_converter_pat.format(attr_name)
            return "self.{attr_name} = {conv}({value_var})".format(
                attr_name=attr_name, value_var=value_var, conv=conv_name,
            )
    ...
    if kw_only_args:
        ...
        args += "{leading_comma}*, {kw_only_args}".format(
            leading_comma=", " if args else "", kw_only_args=", ".join(kw_only_args),
        )
    return (
        """\
...
Attribute = _add_hash(
    _add_eq(_add_repr(Attribute, attrs=_a), attrs=_a), attrs=[a for a in _a if a.hash],
)
...
class _CountingAttr:
    ...
    *Internal* data structure of the attrs library.  Running into is most
    ...
    __attrs_attrs__ = tuple(
        ...
        for name in ("counter", "_default", "repr", "eq", "order", "hash", "init",)
    ) + (
        Attribute(
            name="metadata",
            default=None,
...
def make_class(name, attrs, bases=(object,), **attributes_arguments):
    ...
    type_ = type(
        name, bases, {} if post_init is None else {"__attrs_post_init__": post_init},
    )
    ...
    attributes_arguments["eq"], attributes_arguments["order"] = _determine_eq_order(
