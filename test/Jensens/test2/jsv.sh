...
prop="${prop} -Dvds.db.vds=file" 
prop="${prop} -Dvds.db.file.store=$here/vds.db"
prop="${prop} -Dvds.db.tc=$here/tc.data"
prop="${prop} -Dvds.db.pool=$poolconfig"
...
echo ">>"
set -x
$VDS_HOME/bin/gencdag $prop --o $dstsite --p $runsite --dir hw --dax hw.dax
rc=$?
set +x
if [ $rc -ne 0 ]; then
