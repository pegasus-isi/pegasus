...
public class ProfileParserTest {
    ...
    @Test
    public void testParse_quotedValueContainingCommaAndSemicolonStaysSingleProfile()
            throws ProfileParserException {
        List result = ProfileParser.parse("env::KEY=\"a,b;c\"");
        assertThat(result.size(), is(1));
        assertThat(((Profile) result.get(0)).getProfileValue(), is("a,b;c"));
    }

    @Test
    public void testParse_keyWithSlash() throws ProfileParserException {
        List result = ProfileParser.parse("pegasus::transfer/input=\"enabled\"");
        assertThat(result.size(), is(1));
        assertThat(((Profile) result.get(0)).getProfileKey(), is("transfer/input"));
    }

    @Test
    public void testCombine_preservesProfileOrderAcrossNamespaces() {
        List<Profile> profiles = new ArrayList<>();
        profiles.add(new Profile("env", "A", "1"));
        profiles.add(new Profile("condor", "B", "2"));
        profiles.add(new Profile("condor", "C", "3"));

        String result = ProfileParser.combine(profiles);

        assertThat(result, is("env::A=\"1\";condor::B=\"2\",C=\"3\""));
    }

    @Test
    public void testCombine_emptyProfilesObjectReturnsEmptyString() {
        Profiles profiles = new Profiles();
        assertThat(ProfileParser.combine(profiles), is(""));
    }
}
