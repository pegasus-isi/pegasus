...
public class StageOutTest {
     ...
     */
    @Test
    public void testDirectorySharedFSStageOutToOutputSite() {
        mLogger.logEventStart(
                "test.transfer.generator.stageout", "set", Integer.toString(mTestNumber++));
        ...
        FileTransfer actualOutput = (FileTransfer) localStageOutFtxs.toArray()[0];
        // source is still the raw directory - the job's own compute execution wrote it there,
        // and pegasus-transfer's own tar step (not this test) is what tars it
        assertThat(
                actualOutput.getSourceURL().getValue(),
                is("gsiftp://compute.isi.edu/workflows/compute/shared-scratch/./f.out"));
        // destination and registration URL both must be the .tar.gz artifact
        assertThat(
                actualOutput.getDestURL().getValue(),
                is("gsiftp://output.isi.edu/workflows/output/f.out.tar.gz"));
        assertThat(
                actualOutput.getURLForRegistrationOnDestination(),
                is("gsiftp://output.isi.edu/workflows/output/f.out.tar.gz"));
