...
public class ReplicaLocationTest {
    ...
    public void serializerReplicaLocation() throws IOException {
        ...
        String expected =
                "---\n"
                        ...
                        + "metadata:\n"
                        + "  foo: \"bar\"\n";
        assertThat(mapper.writeValueAsString(rl), is(expected));
    }
    ...
    @Test
    public void constructorMovesNonSiteAttributesIntoMetadata() {
        List<ReplicaCatalogEntry> pfns = new ArrayList<>();
        ReplicaCatalogEntry rce = new ReplicaCatalogEntry("file:///tmp/f.txt", "local");
        rce.addAttribute("owner", "pegasus");
        pfns.add(rce);

        ReplicaLocation rl = new ReplicaLocation("f.txt", pfns);

        assertThat(rl.getMetadata("owner"), is("pegasus"));
        assertThat(rl.getPFN(0).getAttribute("owner"), nullValue());
    }

    @Test
    public void addPFNWithoutSanitizeKeepsNullSiteHandle() {
        ReplicaLocation rl = new ReplicaLocation();
        ReplicaCatalogEntry rce = new ReplicaCatalogEntry("file:///tmp/f.txt");

        rl.addPFN(rce, false);

        assertThat(rl.getPFN(0).getResourceHandle(), nullValue());
    }
    ...
    @Test
    public void mergePurgesExistingMetadataWhenExistingPfnsAreFullyReplaced() {
        List<ReplicaCatalogEntry> pfns1 = new ArrayList<>();
        pfns1.add(new ReplicaCatalogEntry("file:///a", "local"));
        ReplicaLocation rl1 = new ReplicaLocation("f1", pfns1);
        rl1.addMetadata("owner", "original");
        ...
        assertThat(rl1.getMetadata("owner"), nullValue());
    }

    @Test
    public void mergeDoesNotCopyIncomingMetadataToExistingLocation() {
        List<ReplicaCatalogEntry> pfns1 = new ArrayList<>();
        pfns1.add(new ReplicaCatalogEntry("file:///a", "local"));
        ReplicaLocation rl1 = new ReplicaLocation("f1", pfns1);
        ...
        pfns2.add(new ReplicaCatalogEntry("gsiftp://b/f1", "remote"));
        ReplicaLocation rl2 = new ReplicaLocation("f1", pfns2);
        rl2.addMetadata("owner", "incoming");

        rl1.merge(rl2);

        assertThat(rl1.getMetadata("owner"), nullValue());
        assertThat(rl2.getMetadata("owner"), is("incoming"));
    }
    ...
    @Test
    public void deserializerRequiresLfn() throws IOException {
        ...
        assertThrows(
                ReplicaCatalogException.class, () -> mapper.readValue(test, ReplicaLocation.class));
    }

    @Test
    public void deserializerRejectsRegexReplicaWithMultiplePfns() {
        ObjectMapper mapper = new ObjectMapper(new YAMLFactory());
        mapper.configure(MapperFeature.ALLOW_COERCION_OF_SCALARS, false);
