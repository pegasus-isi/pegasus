...
public class CompoundTransformationTest {
    ...
    @Test
    public void testDependantFilesPreserveInsertionOrder() {
        CompoundTransformation ct = new CompoundTransformation("t");
        PegasusFile first = new PegasusFile("a.txt");
        PegasusFile second = new PegasusFile("b.txt");
        ct.addDependantFile(first);
        ct.addDependantFile(second);

        assertThat(ct.getDependantFiles().get(0), is(first));
        assertThat(ct.getDependantFiles().get(1), is(second));
    }
    ...
    public void testEqualsSymmetric() {
        ...
        assertThat(ct1, is(ct2));
        assertThat(ct2, is(ct1));
    }
    ...
    @Test
    public void testEqualsIgnoresDependantFilesAndNotifications() {
        CompoundTransformation ct1 = new CompoundTransformation("ns", "name", "1.0");
        CompoundTransformation ct2 = new CompoundTransformation("ns", "name", "1.0");

        ct1.addDependantFile(new PegasusFile("a.txt"));
        ct1.addNotification(new Invoke(Invoke.WHEN.start, "/one.sh"));

        ct2.addDependantFile(new PegasusFile("b.txt"));
        ct2.addNotification(new Invoke(Invoke.WHEN.error, "/two.sh"));
    ...
    @Test
    public void testGetNotificationsReturnsUnderlyingContainer() {
        CompoundTransformation ct = new CompoundTransformation("t");
        ct.addNotification(new Invoke(Invoke.WHEN.start, "/start.sh"));

        assertThat(ct.getNotifications(), is(notNullValue()));
        assertThat(ct.getNotifications(Invoke.WHEN.start), hasSize(1));
    }
