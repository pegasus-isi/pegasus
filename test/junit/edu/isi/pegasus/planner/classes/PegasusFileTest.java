...
public class PegasusFileTest {
    ...
    @Test
    public void testSetTransferFlagStringNullDefaultsToMandatory() {
        PegasusFile pf = new PegasusFile("f.txt");
        pf.setTransferFlag(PegasusFile.TRANSFER_NOT);

        pf.setTransferFlag((String) null);

        assertEquals(PegasusFile.TRANSFER_MANDATORY, pf.getTransferFlag());
    }

    @Test
    public void testSetTransferFlagStringEmptyDefaultsToMandatory() {
        PegasusFile pf = new PegasusFile("f.txt");
        pf.setTransferFlag(PegasusFile.TRANSFER_OPTIONAL);
    ...
    @Test
    public void testSetTransientRegFlagSetsDoNotRegisterBit() {
        PegasusFile pf = new PegasusFile("f.txt");
    ...
    @Test
    public void testSetRawInputFalseStillMarksFileAsRawInput() {
        PegasusFile pf = new PegasusFile("f.txt");

        pf.setRawInput(false);

        assertTrue(pf.isRawInputFile());
    }

    @Test
    public void testSetTypeOtherString() {
        PegasusFile pf = new PegasusFile("f.txt");

        pf.setType(PegasusFile.OTHER_TYPE);

        assertEquals(PegasusFile.OTHER_FILE, pf.getType());
        assertEquals(PegasusFile.OTHER_TYPE, pf.getTypeAsString());
        assertEquals(PegasusFile.OTHER_TYPE, pf.typeToString());
    }
    ...
    public void testEqualsReturnsFalseForNullAndDifferentType() {
        ...
        assertFalse(pf.equals("data.txt"));
    }

    @Test
    public void testCloneDeepCopiesFlagsAndMetadata() {
        PegasusFile pf = new PegasusFile("original.txt");
        pf.setFileOptional(true);
        pf.addMetadata("creator", "pegasus");

        PegasusFile clone = (PegasusFile) pf.clone();
        clone.setFileOptional(false);
        clone.addMetadata("creator", "other");

        assertTrue(pf.fileOptional());
        assertEquals("pegasus", pf.getMetadata("creator"));
        assertFalse(clone.fileOptional());
        assertEquals("other", clone.getMetadata("creator"));
    }

    @Test
    public void testCloneResetsLinkageToNone() {
        PegasusFile pf = new PegasusFile("original.txt");
        pf.setLinkage(PegasusFile.LINKAGE.output);

        PegasusFile clone = (PegasusFile) pf.clone();

        assertEquals(PegasusFile.LINKAGE.none, clone.getLinkage());
    }

    @Test
    public void testDeserializationWithIllegalUsesKeyThrowsRuntimeException() {
        ObjectMapper mapper = new ObjectMapper(new YAMLFactory());
        mapper.configure(MapperFeature.ALLOW_COERCION_OF_SCALARS, false);

        String test = "lfn: f.b2\n" + "bogus: true\n";

        assertThrows(RuntimeException.class, () -> mapper.readValue(test, PegasusFile.class));
    }
}
