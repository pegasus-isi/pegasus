...
public class BlackDiamondDAX {
    ...
    public static void main(String[] args) {
        if (args.length != 2) {
            System.out.println("Usage: java ADAG <pegasus_location> <filename.dax>");
            System.exit(1);
        }
    ...
    private static ADAG Diamond(String pegasus_location) throws Exception {
        ...
        preprocess.setArchitecture(Executable.ARCH.X86_64).setOS(Executable.OS.LINUX).setOSRelease("deb").setOSVersion("7");
        ...
        findrange.setArchitecture(Executable.ARCH.X86_64).setOS(Executable.OS.LINUX).setOSRelease("deb").setOSVersion("7");
        findrange.setInstalled(true);
        findrange.addPhysicalFile("file://" + pegasus_location + "/bin/pegasus-keg", "local");

        Executable analyze = new Executable("pegasus", "analyze", "4.0").setOSRelease("deb").setOSVersion("7");
        ...
        j1.addArgument("-a preprocess -T 2 -i ").addArgument(fa);
        ...
        j2.addArgument("-a findrange -T 2 -i ").addArgument(fb1);
        ...
        j3.addArgument("-a findrange -T 2 -i ").addArgument(fb2);
        ...
        j4.addArgument("-a analyze -T 2 -i ").addArgument(fc1);
