...
char **av;
{
    double ra, dec, spd, mj, mh, mk, rad;
    int i, j;
    char root[] = "/data/astrocat/tmx";
    char dir[256];
	...
	if (access (dir, F_OK&W_OK)) {
	    if (mkdir (dir, 00777))
		fprintf (stderr, "TMXCAT: Cannot make directory %s\n", dir);
	    }
	}
	...
	if (strcmp (file, file0)) {
	    /* if (nz > 1)
		printf (" TMXCAT: %7d stars written to %s\n", nz, file0); */
	    if (nz == 1) {
	/* 	printf (" TMXCAT: %7d stars written to %s", nz, file0);
		...
		if (outfile == NULL) {
		    fprintf (stderr, "TMXCAT: Cannot write line %d to file %s\n", ni,file);
		    if (dumpfile != NULL)
			fclose (dumpfile);
		    dumpfile = fopen ("badlines", "a");
		    if (dumpfile == NULL)
			dumpfile = fopen ("badlines", "w");
		    fprintf (dumpfile,"%s\n", line);
		    continue;
		    }
		}
	...
	/* Right ascension */
	getoken (&tokens, 3, token, 16);
	ra = atof (token);

	/* ID */
	getoken (&tokens, 2, id, 32);

	/* J, H, K magnitudes */
	getoken (&tokens, 11, token, 16);
	mj = atof (token);
	getoken (&tokens, 14, token, 16);
	mh = atof (token);
	getoken (&tokens, 17, token, 16);
	mk = atof (token);

	/* Isophotal semi-major axis */
	getoken (&tokens, 10, token, 16);
	rad = atof (token);
	...
	if (use_src) {
	    ...
	    fprintf (outfile,"%10.6f %10.6f %17s %6.3f %6.3f %6.3f %6.1f\n",
		 ra, dec, id, mj, mh, mk, rad);
	    nt = nt + 1;
	    if (nt % 100000 == 0)
...
/* Jan  9 2004	New program, based on tmcat.c
 */
