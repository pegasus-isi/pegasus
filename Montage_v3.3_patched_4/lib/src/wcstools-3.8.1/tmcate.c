...
char **av;
{
    double ra, dec, spd, mj, mh, mk, ej, eh, ek;
    int i, j;
    char root[] = "/data/astrocat2/tmce";
    char dir[256];
	...
	if (access (dir, F_OK&W_OK)) {
	    if (mkdir (dir, 00777))
		fprintf (stderr, "TMCATE: Cannot make directory %s\n", dir);
	    }
	}
	...
	if (strcmp (file, file0)) {
	    /* if (nz > 1)
		printf (" TMCATE: %7d stars written to %s\n", nz, file0); */
	    if (nz == 1) {
	/* 	printf (" TMCATE: %7d stars written to %s", nz, file0);
		...
		if (outfile == NULL) {
		    printf ("TMCATE: Cannot write to file %s\n", file);
		    continue;
		    }
		}
	    strcpy (file0, file);
	...
	/* J, H, K errors */
	getoken (&tokens, 9, token, 16);
	ej = atof (token);
	getoken (&tokens, 13, token, 16);
	eh = atof (token);
	...
	if (use_src) {
	    ...
	    fprintf (outfile,"%10.6f %10.6f %17s %6.3f %6.3f %6.3f %5.3f %5.3f %5.3f %3s %3s\n",
		 ra, dec, id, mj, mh, mk, ej, eh, ek, f1, f2);
	    nt = nt + 1;
	    if (nt % 100000 == 0)
...
/* Mar 11 2005	New program, based on tmcat.c
 */
