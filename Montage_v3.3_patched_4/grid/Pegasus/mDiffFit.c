...
int main(int argc, char **argv)
{
   ...
   path[0] = '\0';   
   if ( getenv("MONTAGE_BIN") != NULL ) {
      strcpy(path, getenv("MONTAGE_BIN"));
   }
   else {
      if ( getenv("MONTAGE_HOME") != NULL ) {
         strcpy(path, getenv("MONTAGE_HOME"));
         strcat(path, "/bin");
      }
   }
   ...
   if(noAreas)
      if(strlen(path) > 0)
         sprintf(cmd, "%s/mDiff -n %s %s %s %s", path, input_file1, input_file2,
      ...
      if(strlen(path) > 0)
         sprintf(cmd, "%s/mDiff %s %s %s %s", path, input_file1, input_file2,
            output_file, template_file);
      else 
   ...
   if(strlen(path) > 0)
      sprintf(cmd, "%s/mFitplane -b %d %s", path, border, output_file);
   else
