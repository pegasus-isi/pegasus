...
a = File("input_file")
a.addPFN(PFN("file://" + os.getcwd() + "/input.txt", "local"))
root_dax.addFile(a)
...
e_generate_diamond = Executable(namespace="diamond", name="generate",  os="linux", arch="x86_64", installed=True)
e_generate_diamond.addPFN(PFN("file://" + os.getcwd() + "/sub-workflow.py", "local"))
root_dax.addExecutable(e_generate_diamond)

# Add a remote or local split job
split = Job(namespace="linux", name="split")
b = File("output.list")
split.addArguments("-l 1",a,"output.txt.")
...
generate = Job(namespace="diamond", name="generate")
