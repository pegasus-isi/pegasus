...
1. **Set up temp files** via `bls_setup_temp_files` (deliberately avoids `bls_setup_all_files`
   because stdin/stdout/stderr are handled remotely, not via a local sandbox).
1. **Build the SBATCH script** in `$bls_tmp_file`:
...
1. **Collect output files** from `$bls_opt_outputflstring`.
1. **Submit** by calling:
   ```bash
   python3 sfapi_helpers.py submit \
       ...
       --script      "$bls_tmp_file"
   ```
1. **Parse the result line** `SFAPI_RESULT:<jobid>:<remote_stdout>:<remote_stderr>`.
1. **Write the jobstate file** at `~/.blah/sfapi_jobs/<date>_<jobID>`:
   ```
...
1. **Output the blahp job ID**: `BLAHP_JOBID_PREFIXsfapi/<YYYYMMDD>/<jobID>`
...
1. Query: `python3 sfapi_helpers.py status --type job --value $JOBID`
1. Parse `"Job <id> state: <state>"` from output.
1. Map Slurm/SFAPI state to blahp status code via `map_sfapi_state_to_blahp`:
...
1. If output contains `sfapi_client.exceptions.SfApiError: Job not found:`, treat as success
...
1. SFAPI credentials at `~/.superfacility/`:
...
1. HTCondor `blah.config` must reference the sfapi plugin. The `sfapi_resource` variable
