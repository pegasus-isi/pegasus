...
# +remote_cerequirements = .. REQUEST=="A=\"XY Z\",B=LMN"
# The temp ce-req-file generated by condor writes it as -> REQUEST='"A=XY Z,B=LMN"'
...
if [ -n "$REQUEST" ] ; then
    ...
    echo "#BSUB -csm y"
    ####
    eval "STRIPPED_REQUEST=$REQUEST"
    echo "#BSUB -R ${STRIPPED_REQUEST}"
fi

if [ -n "$ALLOC_FLAGS" ]; then
    eval "STRIPPED_ALLOC_FLAGS=$ALLOC_FLAGS"
    echo "#BSUB -alloc_flags ${STRIPPED_ALLOC_FLAGS}"
fi
