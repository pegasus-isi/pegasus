...
class WorkflowLoader(BaseLoader):
    ...
    def __init__(
        ...
        """
        super(WorkflowLoader, self).__init__(
            connString,
            batch=batch,
            props=props,
            db_type=db_type,
        ...
        for retry in range(1, self.MAX_RETRIES + 1):
            ...
            try:
                ...
                if linedata["event"].startswith("stampede.job_inst."):
                    self.log.warning(
                        'Corner case jobstate event: "%s"', linedata["event"]
                    )
                    self.jobstate(linedata)
                else:
                    self.log.error(
                        'No handler for event type "%s" defined', linedata["event"]
                    )
    ...
    def hard_flush(self, batch_flush=True, retry=0):
        ...
        self.log.debug(
            "Batch event sizes: batch_event_size=%s update_event_size=%s",
        ...
        try:
            ...
            self.log.exception(e)
            self.log.error(
                "Integrity error on batch flush: batch will need to be committed per-event which will take longer"
            )
            self.session.rollback()
        ...
        try:
            ...
            self.log.exception(e)
            self.log.error(
                "Connection problem on host_map_events during commit in hard_flush()"
            )
            self.session.rollback()
    ...
    def job_instance(self, linedata):
        ...
        job_instance.job_id = self.get_job_id(
            job_instance.wf_id, job_instance.exec_job_id
        )
        if not job_instance.job_id:
            self.log.error(
                "Could not determine job_id for job_instance: %s", job_instance
            )
            return
    ...
    def host(self, linedata):
        ...
        if (
            ...
            host.hostname,
            host.ip,
        ) not in self.hosts_written_cache:
            if self._batch:
                self._batch_cache["batch_events"].append(host)
            else:
                host.commit_to_db(self.session)
            self.hosts_written_cache[
                (host.wf_id, host.site, host.hostname, host.ip)
            ] = True
    ...
    def get_lfn_id(self, wf_id, lfn):
        ...
        if (wf_id, lfn) not in self.lfn_id_cache:
            ...
            if id is None:
                ...
                if id is None:
                    self.log.error(
                        "No results found for wf_uuid/lfn: %s/%s", wf_id, lfn
                    )
                    return None
    ...
    def get_job_instance_id(self, o, quiet=False):
        ...
        if uniqueIdIdx not in self.job_instance_id_cache:
            ...
            try:
                ...
                if not quiet:
                    self.log.error(
                        "No job_instance_id results for tuple %s : %s", uniqueIdIdx, e
                    )
                return None
