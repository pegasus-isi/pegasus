    ...
    def update(self, force=False):
        ...
        with warnings.catch_warnings():
            ...
            if set(tables_to_inquiry).issubset(table_names):
                ...
                if not mw_id.references(meta.tables['master_workflow'].c.wf_id) and mw_id.references(meta.tables['workflow'].c.wf_id):
                    ...
                    if self.db.get_bind().driver == "mysqldb":
                        ...
                        self.db.execute("ALTER TABLE master_workflowstate RENAME TO master_workflowstate_v4")
                    pg_workflowstate.create(self.db.get_bind(), checkfirst=True)
                    self.db.execute("INSERT INTO master_workflowstate(wf_id, state, timestamp, restart_count, status) SELECT m4.wf_id, m4.state, m4.timestamp, m4.restart_count, m4.status FROM master_workflowstate_v4 m4 LEFT JOIN master_workflow mw WHERE m4.wf_id=mw.wf_id")
                    self.db.commit()
