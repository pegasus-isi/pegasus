...
class Version(BaseVersion):
    ...
    def update(self, force=False):
        ...
        try:
            ...
            self.db.execute(
                "CREATE TABLE tags ( tag_id INTEGER NOT NULL, wf_id INTEGER NOT NULL, job_instance_id INTEGER NOT NULL, type VARCHAR(255) NOT NULL, count INTEGER NOT NULL, PRIMARY KEY (tag_id), FOREIGN KEY(wf_id) REFERENCES workflow (wf_id) ON DELETE CASCADE, FOREIGN KEY(job_instance_id) REFERENCES job_instance (job_instance_id) ON DELETE CASCADE )"
            )
