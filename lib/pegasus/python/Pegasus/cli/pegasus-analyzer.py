...
def get_pegasus_lite_wrapper(my_job):
    ...
    pegasus_lite_wrapper = os.path.join( os.path.dirname( my_job.sub_file), my_job.name + ".sh")
...
def generate_pegasus_lite_debug_wrapper(pegasus_lite_wrapper):
    ...
    debug_wrapper= os.path.join( debug_dir, "stripped_pl_" + os.path.basename( pegasus_lite_wrapper ))
    try:
        ...
        for line in WRAPPER:
            ...
            if debug_job_local_executable is not None:
                ...
                if ks_invocation and ks_invocation.groups() > 0:
                    logger.debug("Match found for kickstart invocation in pegasus lite wrapper %s" %ks_invocation.group(0))
                    logger.debug("Executable to be replaced is %s" %ks_invocation.group(1))
                    substituted_string = line[:ks_invocation.start(1)] + debug_job_local_executable + line[ks_invocation.end(1):]
        ...
        for entry in my_output:
            ...
            if "stdout" in entry:
                if len(entry["stdout"]) > 0:
                    ...
                    print_console(  ("Task #" + str(my_task_id) + " - " + entry["transformation"] + " - " + entry["derivation"] + " - stdout").center(80, '-') )
                    print_console()
            ...
            if "stderr" in entry:
                if len(entry["stderr"]) > 0:
                    # Something to display
                    print_console(  ("Task #" + str(my_task_id) + " - " + entry["transformation"] + " - " + entry["derivation"] + " - stderr").center(80, '-') )
                    print_console()
...
def print_job_info(job):
    ...
    if print_invocation:
        print_console( )
        print_console(  "To re-run this job, use: %s %s" % (jobs[job].executable, jobs[job].arguments) )
        print_console()
    ...
    if len(jobs[job].dagman_out) > 0:
        ...
        sub_wf_cmd = "%s %s -d %s" % (user_cmd, extraOptions, os.path.split(jobs[job].dagman_out)[0])
...
def check_for_wf_start():
    ...
    if unsubmitted == total:
        ...
        if input_dir is not None:
            dagman_out = backticks( "ls " + input_dir + "/*.dag.dagman.out" + " 2>/dev/null" )

            if dagman_out is not None and dagman_out != "":
                nfs_error_string = backticks( "grep -i \".*Error.*NFS$\" " + dagman_out + " 2>/dev/null" )
                if( nfs_error_string is not None and nfs_error_string != "" ):
                    ...
                    print_console( " HTCondor DAGMan NFS ERROR condition detected in " + dagman_out)
                    print_console( " " + nfs_error_string )
                    print_console( " HTCondor DAGMan expects submit directories to be NOT NFS mounted" )
                    print_console( " Set your submit directory to a directory on the local filesystem OR ")
                    print_console( "    Set HTCondor configuration CREATE_LOCKS_ON_LOCAL_DISK and ENABLE_USERLOG_LOCKING to True. Check HTCondor documentation for further details.")
                    print_console( )

            #PM-1040 check for dagman.lib.err
            dagman_lib_err = backticks( "ls " + input_dir + "/*.dag.lib.err" + " 2>/dev/null" )
            if dagman_lib_err is not None and dagman_lib_err != "":
                dagman_lib_err_contents = backticks( "cat " + dagman_lib_err + " 2>/dev/null")
                if( dagman_lib_err_contents is not None and dagman_lib_err_contents != "" ):
...
def print_top_summary():
    ...
    print_console( " Total jobs         : % 6d (%3.2f%%)" % (total, 100 * (1.0 * total/(total or 1))) )
    print_console( " # jobs succeeded   : % 6d (%3.2f%%)" % (success, 100 * (1.0 * success/(total or 1))) )
    print_console( " # jobs failed      : % 6d (%3.2f%%)" % (failed, 100 * (1.0 * failed/(total or 1))) )
    print_console( " # jobs held        : % 6d (%3.2f%%)" % (held, 100 * (1.0 * held/(total or 1))) )
    print_console( " # jobs unsubmitted : % 6d (%3.2f%%)" % (unsubmitted, 100 * (1.0 * unsubmitted/(total or 1))) )
    if unknown > 0:
        print_console( " # jobs unknown     : % 6d (%3.2f%%)" % (unknown, 100 * (1.0 * unknown/(total or 1)))  )
    print()
...
def analyze_db(config_properties):
    ...
    if held > 0:
        ...
        for held_job in held_jobs:
            ...
            print_console()
            print_console( "submit file            : %s" % (held_job.jobname + ".sub" or '-') )
    ...
    if failed > 0:
        ...
        for my_job in my_failed_jobs:
            ...
            my_info_stdout_text = utils.unquote(my_info.stdout_text or "").decode('UTF-8')
            my_info_stderr_text = utils.unquote(my_info.stderr_text or "").decode('UTF-8')
            ...
            if my_job[0] == my_info.job_instance_id:
                ...
                if print_invocation:
                    print_console()
                    print_console( "To re-run this job, use: %s %s" % ((my_info.executable or '-'), (my_info.argv or '-')) )
                    print_console()
                if print_pre_script and len((my_info.pre_executable or "")) > 0:
                    print_console()
                    print_console( "SCRIPT PRE:" )
                    print_console( "%s %s" % ((my_info.pre_executable or ""), (my_info.pre_argv or "")) )
                    print_console()
                if my_info.subwf_dir is not None:
                    ...
                    #get any options that need to be invoked for the sub workflow
                    extraOptions = addon( options )
                    sub_wf_cmd = "%s %s -d %s --top-dir %s" % (user_cmd, extraOptions, my_wfdir, (top_dir or input_dir))
                ...
                for my_task in my_tasks:
                    ...
                    if not quiet_mode:
                        ...
                        if my_stdout_start >= 0:
                            ...
                            my_stdout_end = my_info_stdout_text .find("#@", my_stdout_start)
                            if my_stdout_end < 0:
                            ...
                            if my_stdout_end - my_stdout_start > 0:
                                ...
                                print_console( ("Task #" + str(my_task[0]) + " - " + str(my_task[4]) + " - " + str(my_task[5]) + " - stdout").center(80, '-') )
                                print_console()
                                print_console( my_info_stdout_text [my_stdout_start:my_stdout_end] )
                                print_console()
                        ...
                        if my_stderr_start >= 0:
                            ...
                            my_stderr_end = my_info_stdout_text .find("#@", my_stderr_start)
                            if my_stderr_end < 0:
                                # Next comment not found, possibly the last entry
                            ...
                            if my_stderr_end - my_stderr_start > 0:
                                # Something to display
                                my_print_job_stderr = False
                                print_console( ("Task #" + str(my_task[0]) + " - " + str(my_task[4]) + " - " + str(my_task[5]) + " - Kickstart stderr").center(80, '-') )
                                print_console()
                                print_console( my_info_stdout_text [my_stderr_start:my_stderr_end] )
                                print_console( )
                ...
                if my_info_stderr_text.strip("\n\t \r") != "" and my_print_job_stderr:
                    ...
                    print_console( ("Job stderr file - %s" % (my_info.stderr_file or '-')).center(80, '-')  )
                    print_console()
...
if [ -e $file ]; then
    ...
    STATUS=$(($STATUS + 1))
        ...
        if( job_pegasus_lite_wrapper is None ):
            ...
            debug_wrapper = generate_pegasus_lite_debug_wrapper( job_pegasus_lite_wrapper )
        ...
        if( job_pegasus_lite_wrapper is not None):
            debug_script.write( " 2> " + os.path.join(debug_dir, debug_script_err_basename) )
...
parser.add_option("-r", "--recurse", action = "store_true",  dest = "recurse_mode",
...
parser.add_option("--local-executable", action = "store", type = "string", dest = "debug_job_local_executable",
