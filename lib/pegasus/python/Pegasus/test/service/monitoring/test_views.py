...
class FlaskTestCase(unittest.TestCase, JSONResponseMixin):
    ...
    def get_with_context(self, uri, data=None, pre_callable=None):
        return self.request_with_context(
            uri, 'GET', data, pre_callable=pre_callable
        )

    def post_with_context(self, uri, data=None, pre_callable=None):
        return self.request_with_context(
            uri, 'POST', data, pre_callable=pre_callable
        )

    def put_with_context(self, uri, data=None, pre_callable=None):
        return self.request_with_context(
            uri, 'PUT', data, pre_callable=pre_callable
        )

    def delete_with_context(self, uri, data=None, pre_callable=None):
        return self.request_with_context(
            uri, 'DELETE', data, pre_callable=pre_callable
        )

    def request_with_context(
        self, uri, method='GET', data=None, pre_callable=None
    ):
        """
...
class TestMasterWorkflowQueries(NoAuthFlaskTestCase):
    ...
    def test_ambiguous_query(self):
        rv = self.get_context(
            '/api/v1/user/%s/root?query=timestamp > 1000.0&order=wf_id asc' %
            self.user,
            pre_callable=self.pre_callable
        )
    ...
    def test_get_root_workflow_id(self):
        rv = self.get_context(
            '/api/v1/user/%s/root/1' % self.user,
            pre_callable=self.pre_callable
        )
    ...
    def test_get_root_workflow_uuid(self):
        rv = self.get_context(
            '/api/v1/user/%s/root/7193de8c-a28d-4eca-b576-1b1c3c4f668b' %
            self.user,
            pre_callable=self.pre_callable
        )
...
class TestStampedeWorkflowQueries(NoAuthFlaskTestCase):
    def test_get_workflows(self):
        ...
        self.assertEqual(
            len(workflows['records']), workflows['_meta']['records_total']
        )
        self.assertEqual(
    ...
    def test_get_workflow_uuid(self):
        ...
        self.assertTrue(
            workflow['wf_uuid'], '7193de8c-a28d-4eca-b576-1b1c3c4f668b'
        )
...
class TestStampedeWorkflowMetaQueries(NoAuthFlaskTestCase):
    def test_get_workflow_metas(self):
        ...
        self.assertEqual(
            len(metas['records']), metas['_meta']['records_total']
        )
        self.assertEqual(
...
class TestStampedeWorkflowFilesQueries(NoAuthFlaskTestCase):
    def test_get_workflow_files(self):
        ...
        self.assertEqual(
            len(files['records']), files['_meta']['records_total']
        )
        self.assertEqual(
    ...
    def test_get_workflow_files_query(self):
        ...
        files = self.read_json_response(rv)
        self.assertEqual(
            len(files['records']), files['_meta']['records_filtered']
        )
...
class TestStampedeHostQueries(NoAuthFlaskTestCase):
    def test_get_workflow_hosts(self):
        ...
        self.assertEqual(
            len(hosts['records']), hosts['_meta']['records_total']
        )
        self.assertEqual(
...
class TestStampedeJobstateQueries(NoAuthFlaskTestCase):
    def test_get_jobstates(self):
        rv = self.get_context(
            '/api/v1/user/%s/root/1/workflow/1/job/1/job-instance/3/state' %
            self.user,
            pre_callable=self.pre_callable
        )
...
class TestStampedeTaskQueries(NoAuthFlaskTestCase):
    def test_get_workflow_tasks(self):
        ...
        self.assertEqual(
            len(tasks['records']), tasks['_meta']['records_total']
        )
        self.assertEqual(
