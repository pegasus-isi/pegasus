...
class TestQuoting(unittest.TestCase):
    def testQuote(self):
        ...
        self.assertEqual(
            utils.quote(u"warning: unused variable ‘Narr’"),
            "warning: unused variable %E2%80%98Narr%E2%80%99"
        )
        self.assertEqual(
            utils.quote(u"warning: unused variable \u2018Narr\u2019"),
    ...
    def testQuoteInvalidChars(self):
        ...
        self.assertEqual(utils.quote("\x80"),
                         "%80")  # Invalid 1 Octet Sequence
        self.assertEqual(utils.quote("\xc3\x28"),
                         "%C3(")  # Invalid 2 Octet Sequence
        self.assertEqual(utils.quote("\xa0\xa1"),
                         "%A0%A1")  # Invalid Sequence Identifier
        self.assertEqual(utils.quote("\xe2\x82\xa1"),
                         "%E2%82%A1")  # Valid 3 Octet Sequence
        self.assertEqual(utils.quote("\xe2\x28\xa1"),
                         "%E2(%A1")  # Invalid 3 Octet Sequence (in 2nd Octet)
        self.assertEqual(utils.quote("\xe2\x82\x28"),
                         "%E2%82(")  # Invalid 3 Octet Sequence (in 3rd Octet)
        self.assertEqual(utils.quote("\xf0\x90\x8c\xbc"),
                         "%F0%90%8C%BC")  # Valid 4 Octet Sequence
        self.assertEqual(utils.quote("\xf0\x28\x8c\xbc"), "%F0(%8C%BC"
                         )  # Invalid 4 Octet Sequence (in 2nd Octet)
        self.assertEqual(utils.quote("\xf0\x90\x28\xbc"), "%F0%90(%BC"
                         )  # Invalid 4 Octet Sequence (in 3rd Octet)
        self.assertEqual(utils.quote("\xf0\x28\x8c\x28"),
                         "%F0(%8C(")  # Invalid 4 Octet Sequence (in 4th Octet)
        self.assertEqual(
            utils.quote("\xf8\xa1\xa1\xa1\xa1"), "%F8%A1%A1%A1%A1"
        )  # Valid 5 Octet Sequence (but not Unicode!)
        self.assertEqual(
            utils.quote("\xfc\xa1\xa1\xa1\xa1\xa1"), "%FC%A1%A1%A1%A1%A1"
        )  # Valid 6 Octet Sequence (but not Unicode!)

    def testUnquoteInvalidChars(self):
        "Invalid UTF-8 byte strings should not cause unquote to fail"
        self.assertEqual(utils.unquote("%80"),
                         "\x80")  # Invalid 1 Octet Sequence
        self.assertEqual(utils.unquote("%C3("),
                         "\xc3\x28")  # Invalid 2 Octet Sequence
        self.assertEqual(utils.unquote("%A0%A1"),
                         "\xa0\xa1")  # Invalid Sequence Identifier
        self.assertEqual(utils.unquote("%E2%82%A1"),
                         "\xe2\x82\xa1")  # Valid 3 Octet Sequence
        self.assertEqual(utils.unquote("%E2(%A1"), "\xe2\x28\xa1"
                         )  # Invalid 3 Octet Sequence (in 2nd Octet)
        self.assertEqual(utils.unquote("%E2%82("), "\xe2\x82\x28"
                         )  # Invalid 3 Octet Sequence (in 3rd Octet)
        self.assertEqual(utils.unquote("%F0%90%8C%BC"),
                         "\xf0\x90\x8c\xbc")  # Valid 4 Octet Sequence
        self.assertEqual(utils.unquote("%F0(%8C%BC"), "\xf0\x28\x8c\xbc"
                         )  # Invalid 4 Octet Sequence (in 2nd Octet)
        self.assertEqual(utils.unquote("%F0%90(%BC"), "\xf0\x90\x28\xbc"
                         )  # Invalid 4 Octet Sequence (in 3rd Octet)
        self.assertEqual(utils.unquote("%F0(%8C("), "\xf0\x28\x8c\x28"
                         )  # Invalid 4 Octet Sequence (in 4th Octet)
        self.assertEqual(
            utils.unquote("%F8%A1%A1%A1%A1"), "\xf8\xa1\xa1\xa1\xa1"
        )  # Valid 5 Octet Sequence (but not Unicode!)
        self.assertEqual(
            utils.unquote("%FC%A1%A1%A1%A1%A1"), "\xfc\xa1\xa1\xa1\xa1\xa1"
        )  # Valid 6 Octet Sequence (but not Unicode!)
    ...
    def testQuoteUnquoteLatin1(self):
        ...
        self.assertEqual(
            "R\xe9sum\xe9", utils.unquote(utils.quote("R\xe9sum\xe9"))
        )
...
class TestISODate(unittest.TestCase):
    ...
    def testLocal(self):
        ...
        if time.timezone == 28800:
            self.assertEqual(
                "2012-04-17T18:55:32-0700", utils.isodate(now=self.now)
            )
    ...
    def testUTC(self):
        "Long UTC timestamp"
        self.assertEqual(
            "2012-04-18T01:55:32Z", utils.isodate(now=self.now, utc=True)
        )
    ...
    def testShortUTC(self):
        ...
        self.assertEqual(
            "20120418T015532Z",
            utils.isodate(now=self.now, utc=True, short=True)
        )
...
class TestEpochDate(unittest.TestCase):
    ...
    def testLocal(self):
        ...
        self.assertEqual(
            self.now, utils.epochdate(utils.isodate(now=self.now))
        )
    ...
    def testShortUTC(self):
        ...
        self.assertEqual(
            self.now,
            utils.epochdate(utils.isodate(now=self.now, utc=True, short=True))
        )
