package GriPhyN::RC::LRC;
#
# RLS LRC-only based replica manager implementation
#
...
#
# $Id: LRC.pm,v 1.2 2005/09/01 23:46:15 griphyn Exp $
#
use 5.006;
use strict;
use vars qw($VERSION);
...
sub failure {
    # purpose: deal with a failure, signal or non-zero exit
    # globals: $? (IN): contains most recent child exit code
    # paramtr: $app (IN): basename of application that was run
    #          $benign (IN): if true, non-0 exit just warns, no death
    #          ... (IN): the result set
    # returns: only returns in benign case
    my $self = shift;
    my $app = shift;
    my $benign = shift;
    ...
    if ( $? & 127 ) {
	# died on signal
	die( $prefix, "$app died on signal @{[$?&127]}: ",
	     @_ > 5 ? @_[0..5] : @_, "\n" );
    } else {
	# non-zero exit code
	if ( $benign ) {
	    log( $prefix, "$app exited with status @{[$?>>8]}" );
	} else {
	    die( $prefix, "$app exited with status @{[$?>>8]}: ", 
		 @_ > 5 ? @_[0..5] : @_, "\n" );
	}
    }
}
...
sub insert {
	...
	if ( $? == 0 ) {
	    ...
	    # exit with not code 1 is fatal
	    $self->failure( basename($self->{'rls.client'}), 0, @temp );
	}
    }
...
sub remove {
	...
	$self->failure( basename($self->{grc}), 1, @temp ) if $?;
    }
    undef;
}
...
sub bulk_remove {
	...
	unlink($fn);
	unless ( $? == 0 ) {
	    $self->failure( basename($self->{grc}), 1, @temp );
	    undef $result;
	}
    } else {
	# nothing to do
...
sub lookup {
    ...
    my @arg = ( $self->{grc}, 'query', 'lrc', 'lfn', $lfn, $self->{lrc} );
    my @temp = $self->pipe_out_cmd( undef, @arg );
	...
	$self->failure( basename($self->{grc}), 0, @temp )
	    unless ( $? == 256 && $temp[0] =~ /LFN doesn\'t exist/ );
    }
...
sub bulk_lookup {
    ...
    if ( $? == 0 ) {
	# all is well -- walk @temp only once
	foreach ( @temp ) {
	    s/[\r\n ]+$//;	# chomp
	    if ( /^\s+([^:]+)/ ) {
		my $lfn = $1;
		s/^\s+$lfn:\s+//;
		if ( $_ ne 'LFN doesn\'t exist' ) {
		    push( @{$result{$lfn}}, $_ );
		}
	    }
	}
...
sub wildcard_lookup {
    ...
    my @arg = ( $self->{grc}, 'query', 'wildcard', 'lrc', 'lfn', $pattern,
		$self->{lrc} );
    my @temp = $self->pipe_out_cmd( undef, @arg );
	...
	foreach ( @temp ) {
	    s/^\s+//;
	    s/[\r\n ]+$//;	# chomp
	    my ($lfn,$pfn) = split /:\s+/, $_, 2;
	    push( @{$result{$lfn}}, $pfn );
	}
    } else {
	# first case is the OK case
	$self->failure( basename($self->{grc}), 0, @temp )
	    unless ( $? == 256 && $temp[0] =~ /LFN doesn\'t exist/ );
    }
