...
def print_workflow_details(workflow_stat_list ,output_dir):
	...
	for workflow_stat in workflow_stat_list:
		...
		try:
			...
			fh.write( "\n")
			fh.write( "var data = [")
			job_info = workflow_stat.get_formatted_host_data()
			fh.write(job_info)
			fh.write( "];")
		except IOError:
...
function showState(){\n\
if(condorTime || kickstart || condorRuntime || resourceDelay){\n\
return true;\n\
}else{\n\
return false;\n\
}\n\
...
var nameMargin  = 400;\n\
var scaleMargin = 15;\n"
	var_str += "var initMaxY = "+str(number_of_hosts) + "*bar_spacing;\n"
	color_name_str = "var color =['yellow','orange' ,'steelblue'"
	desc_name_str = "var desc=['condor job','resource delay', 'job runtime as seen by dagman'"
	...
	if number_of_hosts < 5:
		var_str +="var h = " +str(number_of_hosts) +"*bar_spacing*2 + scaleMargin + bar_spacing;\n"
	else:
		var_str +="var h = 840;\n"			
	var_str +="var w = 1460;\n\
...
/*barPanel.add(pv.Bar)\n\
.data(function(d) d.jobs)\n\
.visible(function(d){return showState() && barvisibility(d.preS , this.parent.index);})\n\
.height(function(){return inner_bar_width;})\n\
.bottom(inner_bar_margin)\n\
.width(function(d) {\n\
return getPreTime(d);})\n\
...
/*barPanel.add(pv.Bar)\n\
.data(function(d) d.jobs)\n\
.visible(function(d){return showState() && barvisibility(d.postS , this.parent.index);})\n\
.width(function(d) {\n\
